...
Implementation update, 2026-09-23: the original v1 prose envelope below is preserved as design history, but is not accepted by the verifier. The reviewed v2 envelope permits only exact source-bound values and references, rendered deterministically. Seal-marker presence is not authentication; the caller must authenticate the snapshot first. See `scripts/fixbench/README.md` for the current executable contract and `docs/ai-layer-current-status.md` for release status. The trace helper is isolated in `app/ai_trace.py`; shared observability is unchanged.
