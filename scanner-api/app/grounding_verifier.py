...
from .ai_schemas import AIAnnotationV1, ChatAnswerV1, AIAnnotationV2, ChatAnswerV2, UnknownAISchemaVersion, validate_ai_schema
...
EVIDENCE_SET_VERSION = "ai_evidence_set_v2"
GROUNDING_VERIFIER_VERSION = "grounding_verifier_v2"
...
class GroundingResult(BaseModel):
    ...
    verifier_version: Literal["grounding_verifier_v2"] = GROUNDING_VERIFIER_VERSION
    status: Literal["verified", "redacted", "rejected", "unavailable"]
    schema_version: str = ""
    evidence_fingerprint: str = ""
    verified_payload: dict[str, Any] | None = None
    rendered_annotations: list[str] = Field(default_factory=list)
...
# These paths are the compatibility boundary, not a recursive name search.
# An integrator must authenticate the seal before calling this offline adapter.
_COLLECTION_PARENT_PATHS = (
    (),
    ("review",),
    ("stage3_handoff_v2_source",),
    ("review", "stage3_handoff_v2_source"),
    ("health_score_explanation", "stage3_delivery", "handoff_v2_source"),
)
_COLLECTIONS = _PAGE_CONTAINERS | _FIX_CONTAINERS | frozenset({"root_causes"})
_ROOT_SCALARS = frozenset({
    "status", "scan_status", "health_score", "pages_found", "pages_crawled",
    "pages_retained", "fix_count", "release_gate_eligible", "evidence_quality_blocking",
    "provisional", "authority_verified",
})
_SCALAR_CHILDREN = frozenset({"counts", "metrics", "coverage", "verification"})


def _at_path(source: dict[str, Any], parts: tuple[str, ...]) -> Any:
    node: Any = source
    for part in parts:
...
def _evidence_collections(source: dict[str, Any]):
    for parent in _COLLECTION_PARENT_PATHS:
        node = _at_path(source, parent)
        if not isinstance(node, dict):
            continue
        # Handoff collections are evidence only with the recognized version.
        if parent and parent[-1] in {"stage3_handoff_v2_source", "handoff_v2_source"}:
            if node.get("handoff_version") != _STAGE3_HANDOFF_VERSION:
                continue
        for name in sorted(_COLLECTIONS):
            records = node.get(name)
            if not isinstance(records, list):
                continue
            path = "$"
            for part in (*parent, name):
                path = _path(path, part)
            # Only direct dictionary members count; nested arrays are not records.
            for index, record in enumerate(records):
                if isinstance(record, dict):
                    yield name, record, _path(path, index)


def _evidence_scalars(source: dict[str, Any]):
    for field in sorted(_ROOT_SCALARS):
        value = source.get(field)
        if _scalar(value):
            yield _path("$", field), value
    decision_path = ("health_score_explanation", "stage3_delivery", "health_score_decision")
    decision = _at_path(source, decision_path)
    if isinstance(decision, dict):
        for field in ("adjusted_health_score", "coverage_state", "state"):
            value = decision.get(field)
            if _scalar(value):
                yield "$." + ".".join((*decision_path, field)), value
    for _name, record, path in _evidence_collections(source):
        for field, value in record.items():
            if _scalar(value):
                yield _path(path, field), value
            elif field in _SCALAR_CHILDREN and isinstance(value, dict):
                # One defined scalar layer; arbitrary descendants never gain trust.
                for child, scalar in value.items():
                    if _scalar(scalar):
                        yield _path(_path(path, field), child), scalar
...
def build_evidence_set(sealed_l2: dict[str, Any], *, scan_origin: str = "") -> EvidenceSet:
    ...
    members: set[str] = set()
    live: set[str] = set()
    for container, record, _source_path in _evidence_collections(sealed_l2):
        _collect_urls(
            record,
            origin,
            members,
    ...
    for path, value in _evidence_scalars(sealed_l2):
...
def _annotations(model: AIAnnotationV2 | ChatAnswerV2) -> list[AIAnnotationV2]:
    return [model] if isinstance(model, AIAnnotationV2) else list(model.annotations)
...
def _annotation_errors(annotation: AIAnnotationV2, evidence: EvidenceSet) -> list[str]:
    errors: set[str] = set()
...
def _render_verified_annotation(annotation: AIAnnotationV2, evidence: EvidenceSet) -> str:
    """Render verified declarations as literal records, never inferred prose.

    This is an offline debug/acceptance rendering, not a customer explanation.
    Treat it as plain text. The source path supplies the label; there is no model
    label or free-text template slot. Canonical URLs come from evidence identity.
    """
    def quote(value: Any) -> str:
        return json.dumps(value, ensure_ascii=False, allow_nan=False)
    lines = ["Evidence URL: " + quote(_url_key(ref.url, evidence.scan_origin)) + "." for ref in annotation.evidence]
    for claim in [*annotation.numeric_claims, *annotation.state_claims]:
        lines.append("Recorded value at " + quote(claim.source_ref) + ": " + quote(evidence.state_values[claim.source_ref]) + ".")
    lines.extend("Repair reference: " + quote(ref) + "." for ref in annotation.fix_refs)
    lines.extend("Root-cause reference: " + quote(ref) + "." for ref in annotation.root_cause_refs)
    return "\n".join(lines)


def _conflict_key_value(annotation: AIAnnotationV2):
    for claim in annotation.numeric_claims:
...
def _has_deterministic_conflict(annotations: list[AIAnnotationV2]) -> bool:
...
def verify_grounded_payload(
    ...
    *,
    ...
    if isinstance(model, (AIAnnotationV1, ChatAnswerV1)):
        # References alone cannot establish what arbitrary prose claims. No regex
        # or model is used to pretend that v1 text has been semantically verified.
        return GroundingResult(
            status="rejected", schema_version=model.schema_version,
            evidence_fingerprint=evidence_set.fingerprint,
            reasons=["unconstrained_text_unsupported"],
        )
    ...
    invalid: list[tuple[AIAnnotationV2, list[str]]] = []
    valid: list[AIAnnotationV2] = []
    for annotation in annotations:
    ...
    if not invalid:
        return GroundingResult(
            ...
            rendered_annotations=[_render_verified_annotation(item, evidence_set) for item in valid],
        )

    reasons = sorted({reason for _annotation, errors in invalid for reason in errors})
    if isinstance(model, ChatAnswerV2) and valid:
