...
_SUPPORTED_FIXTURES = frozenset({
    "fixbench_grounding_closed_contract_v2",
...
def run_fixture(path: str | Path) -> dict[str, Any]:
    ...
    for case in cases:
        ...
        if "expected_rendered_annotations" in case:
            ok = ok and result.rendered_annotations == case["expected_rendered_annotations"]
        if result.status in {"rejected", "unavailable"}:
            ok = ok and result.verified_payload is None and result.rendered_annotations == []
        passed += int(ok)
        rows.append({
...
def run_suite(directory: str | Path | None = None) -> dict[str, Any]:
    fixture_dir = Path(directory) if directory is not None else Path(__file__).with_name("fixtures")
    paths = sorted(fixture_dir.glob("grounding_*.json"))
    if not paths:
        raise ValueError("FixBench suite is empty")
    reports = [run_fixture(path) for path in paths]
    return {
        "fixbench_version": "fixbench_grounding_v2",
        "gate_state": "passed" if all(report["gate_state"] == "passed" for report in reports) else "failed",
...
def main(argv: list[str]) -> int:
    if len(argv) != 2:
        raise SystemExit("usage: run_grounding.py <fixture.json|--all>")
    report = run_suite() if argv[1] == "--all" else run_fixture(argv[1])
