...
export default function ScanWebsiteForm({ project = null, saving = false }) {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      setError(
        err?.message ||
          "The website scan failed. Try Quick check first or check the backend function logs."
      );
    } finally {
      setActiveStep("");
      setSubmitting(false);
                ...
                return (
                  <button
                    ...
                    className={`rounded-2xl border p-4 text-left transition ${
                      active
                        ? "border-indigo-500 bg-indigo-50 text-slate-950 ring-2 ring-indigo-100"
            ...
            <div className="grid gap-5 rounded-3xl border border-slate-200 bg-slate-50 p-4 lg:grid-cols-2">
              <div>
                ...
                <select
                  ...
                  disabled={isLoading}
                  className="mt-2 h-10 w-full rounded-md border border-slate-200 bg-white px-3 text-sm text-slate-950 shadow-sm outline-none transition focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 disabled:cursor-not-allowed disabled:bg-slate-50"
                >
                  {CMS_OPTIONS.map((cms) => (
...
function buildAiReviewPayload({ scanData, businessName, websiteUrl, cmsPlatform, cmsName, cleanedKeywords, scanMode, requestedPathPrefix }) {
  return {
    ...
    business_priority_instruction: {
      ...
      demotion_rule:
        "Missing meta descriptions, long titles, thin content, or duplicate titles on low-value archive/news pages should usually be low priority and grouped together unless the page is commercially important or has a severe technical error.",
      severe_exception:
        "Broken pages, noindex, canonical mistakes, and redirect loops can stay high or medium if they affect important pages or are linked from important navigation.",
      requested_path_prefix: requestedPathPrefix || "",
    },
    output_requirements: {
...
function simplifyAndPrioritizeFixes(fixes, options = {}) {
  ...
  const grouped = groupLowValueArchiveIssues(enriched);
...
function applyBusinessPriorityRules(fix = {}, options = {}) {
  ...
  const lowValueRule = LOW_VALUE_RULES.has(normalized.rule) || LOW_VALUE_RULES.has(normalized.category);
...
function groupLowValueArchiveIssues(fixes) {
  ...
  return [...keep, ...Array.from(groups.values())];
}
...
function slimAction(action = {}) {
  return {
    ...
    priority: ["critical", "high", "medium", "low"].includes(action.priority) ? action.priority : normalizePriority(action.priority),
...
function isImportantBusinessPage(url = "", options = {}) {
  const normalized = normalizePathText(url);
  if (!normalized) return false;
  const requested = normalizePathText(options.requestedPathPrefix || "");
  if (requested && (normalized === requested || normalized === `${requested}/` || normalized === `${requested}/index.html`)) return true;
  if (/^\/?$/.test(normalized) || normalized.endsWith("/index.html")) return true;
  if (isLowValuePage(normalized)) return false;
  return COMMERCIAL_PAGE_PATTERNS.some((pattern) => normalized.includes(pattern));
}

function isSevereIssue(fix = {}) {
  const rule = String(fix.rule || "").toLowerCase();
  const category = String(fix.category || "").toLowerCase();
  const current = String(fix.current_value || "").toLowerCase();
  if (SEVERE_RULES.has(rule) || SEVERE_RULES.has(category)) return true;
  return ["404", "410", "500", "503"].some((code) => current.includes(code));
}
...
function businessSortScore(fix = {}, options = {}) {
  ...
  const important = fix.is_important_business_page || isImportantBusinessPage(url, options);
  const lowValue = fix.is_low_value_page || isLowValuePage(url);
