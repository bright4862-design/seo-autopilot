...
test("focused scope is bound into authoritative and limited result proofs", () => {
  ...
  assert.equal(limited.includes("standard_limited_result_integrity_v3_focused_scope"), true);
  ...
  assert.equal(reader.includes("standard_limited_result_integrity_v3_focused_scope"), true);
  const projection = fs.readFileSync("base44/functions/getCustomerScanResult/projection.js", "utf8");
  assert.equal(projection.includes('"standard_limited_result_integrity_v3_focused_scope"'), true);
});
