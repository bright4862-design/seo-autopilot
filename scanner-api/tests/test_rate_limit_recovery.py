...
@pytest.mark.parametrize(
    ("retry_after", "expected_waits", "expected_attempts"),
    [
        ("60", [60.0], 2),
        ("Wed, 23 Sep 2026 12:01:00 GMT", [60.0], 2),
        ("invalid", [30.0], 2),
        ("3600", [], 1),
    ],
)
async def test_initial_throttle_respects_retry_after_and_remaining_budget(
    mock_network, monkeypatch, retry_after, expected_waits, expected_attempts, server,
):
