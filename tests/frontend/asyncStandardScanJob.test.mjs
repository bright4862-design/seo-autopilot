...
test("the browser cannot be held open by a crawl", () => {
  ...
  // The synchronous fallback is deleted, not disabled. While it existed it
  // could run a second scanner against the same ScanRun and write that row's
  // terminal state from the browser.
  assert.doesNotMatch(form, /SYNC_FALLBACK_ENABLED/);
  assert.doesNotMatch(form, /callBase44Function\(STANDARD_SCANNER_FUNCTION/);
});
