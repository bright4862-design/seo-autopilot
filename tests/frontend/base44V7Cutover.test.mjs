...
test("V7 packages preserve V6 behavior through delegated legacy copies plus bounded Stage3 additions", () => {
  const allowedExtras = new Map([
    ["persistDurableScanAuthority", new Set([
      "authoritySnapshotStage1Legacy.js",
      "customerPreviewSealStage1Legacy.js",
      "stage3V7Delivery.js",
      "stage3V7DeliveryStrict.js",
    ])],
    ["getCustomerScanResult", new Set([
  ...
  for (const canonical of canonicals) {
    ...
    const v6files = fs.readdirSync(v6dir).sort();
    const v7files = fs.readdirSync(v7dir).sort();
    const extras = allowedExtras.get(canonical) || new Set();
    assert.deepEqual(v7files, [...v6files, ...extras].sort(), canonical);

    for (const file of v6files) {
      if (file === "generatedBuildId.js") continue;
      let actualPath = path.join(v7dir, file);
      if (canonical === "persistDurableScanAuthority" && file === "authoritySnapshot.js") {
        actualPath = path.join(v7dir, "authoritySnapshotStage1Legacy.js");
      } else if (canonical === "persistDurableScanAuthority" && file === "customerPreviewSeal.js") {
        actualPath = path.join(v7dir, "customerPreviewSealStage1Legacy.js");
      } else if (canonical === "getCustomerScanResult" && file === "projection.js") {
        actualPath = path.join(v7dir, "projectionStage1Legacy.js");
      }

      let actual = source(actualPath);
...
test("V7 preserves signed unpaid teaser and shared $49 two-fix contract", () => {
  ...
  const projection = source("base44/functions/getCustomerScanResultV7/projectionStage1Legacy.js");
  const projectionWrapper = source("base44/functions/getCustomerScanResultV7/projection.js");
