...
def test_authenticated_stable_identity_survives_finding_id_changes_without_mutation():
    ...
    assert "different numbers of pages" in result["presentation"]["score_caution"]
...
def test_authenticated_current_finding_without_reference_cannot_disappear_from_customer_counts():
    current_fix = repair("legacy-current", stable=False)
    current_fix.pop("repair_fingerprint")
    pair = inputs(current_fixes=[current_fix])
    with pytest.raises(ValueError, match="cannot all be represented"):
        compare(pair)


@pytest.mark.parametrize("side", ["previous", "current"])
def test_duplicate_reference_rows_cannot_be_collapsed_into_misleading_customer_counts(side):
    pair = inputs(stable=False)
    duplicate = deepcopy(pair[f"{side}_snapshot"]["recommendations"][0])
    duplicate["fix_id"] = "second-row-same-reference"
    pair[f"{side}_snapshot"]["recommendations"].append(duplicate)
    pair[f"{side}_snapshot"]["fix_list"]["total_fixes"] = 2
    sign(pair)
    with pytest.raises(ValueError, match="Duplicate repair references"):
        compare(pair)
