#!/usr/bin/env node
/**
 * Generate every runtime release contract from the one canonical input.
 *
 * Base44 function packages cannot import `../`, so each package that needs the
 * release contract receives its own generated, package-local file. The frontend
 * gets one too. All of them are derived from `data/beta-crawler-revision.json`
 * (the recorded candidate) plus `data/cross-runtime-release-components.json`
 * (the cross-runtime markers Python cannot import), so a copied literal can
 * never drift away from release truth again.
 *
 *   node scripts/generate_release_contracts.mjs            write the contracts
 *   node scripts/generate_release_contracts.mjs --check     fail on any drift
 *   node scripts/generate_release_contracts.mjs --list      list emitted paths
 */

import crypto from "node:crypto";
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";

// RELEASE_CONTRACT_ROOT lets a test drive the generator against an isolated
// copy. Tests run in parallel, so a check that mutated the real tree would race
// with any other test importing a generated contract.
const REPO_ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
const ROOT = process.env.RELEASE_CONTRACT_ROOT
  ? path.resolve(process.env.RELEASE_CONTRACT_ROOT)
  : REPO_ROOT;
const REVISION_RECORD = "data/beta-crawler-revision.json";
const CROSS_RUNTIME_INPUT = "data/cross-runtime-release-components.json";
const CROSS_RUNTIME_SCHEMA = "cross_runtime_release_components_v1";

// Every consumer that needs release identity at runtime. Base44 packages are
// package-local by necessity; the frontend has a single shared module.
// Only packages that actually import the contract are listed: the release
// manifest rejects an unreferenced file as dead bytes shipped through the
// bundler, so an unused generated contract is an integrity failure.
const CONSUMERS = [
  "src/lib/generatedReleaseContract.js",
  "base44/functions/persistDurableScanAuthority/generatedReleaseContract.js",
  "base44/functions/persistLimitedScanResult/generatedReleaseContract.js",
  "base44/functions/startStandardScanJob/generatedReleaseContract.js",
  "base44/functions/durableScanWorkerControl/generatedReleaseContract.js",
  "base44/functions/ownerScanDebugControl/generatedReleaseContract.js",
  "base44/functions/getCustomerScanResult/generatedReleaseContract.js",
  "base44/functions/persistDurableScanAuthorityV2/generatedReleaseContract.js",
  "base44/functions/persistDurableScanAuthorityV3/generatedReleaseContract.js",
  "base44/functions/persistDurableScanAuthorityV4/generatedReleaseContract.js",
  "base44/functions/persistDurableScanAuthorityV5/generatedReleaseContract.js",
  "base44/functions/persistDurableScanAuthorityV6/generatedReleaseContract.js",
  "base44/functions/persistDurableScanAuthorityV7/generatedReleaseContract.js",
  "base44/functions/persistDurableScanAuthorityV8/generatedReleaseContract.js",
  "base44/functions/persistLimitedScanResultV2/generatedReleaseContract.js",
  "base44/functions/persistLimitedScanResultV3/generatedReleaseContract.js",
  "base44/functions/persistLimitedScanResultV4/generatedReleaseContract.js",
  "base44/functions/persistLimitedScanResultV5/generatedReleaseContract.js",
  "base44/functions/persistLimitedScanResultV6/generatedReleaseContract.js",
  "base44/functions/persistLimitedScanResultV7/generatedReleaseContract.js",
  "base44/functions/persistLimitedScanResultV8/generatedReleaseContract.js",
  "base44/functions/startStandardScanJobV2/generatedReleaseContract.js",
  "base44/functions/startStandardScanJobV3/generatedReleaseContract.js",
  "base44/functions/startStandardScanJobV4/generatedReleaseContract.js",
  "base44/functions/startStandardScanJobV5/generatedReleaseContract.js",
  "base44/functions/startStandardScanJobV6/generatedReleaseContract.js",
  "base44/functions/startStandardScanJobV7/generatedReleaseContract.js",
  "base44/functions/startStandardScanJobV8/generatedReleaseContract.js",
  "base44/functions/durableScanWorkerControlV2/generatedReleaseContract.js",
  "base44/functions/durableScanWorkerControlV3/generatedReleaseContract.js",
  "base44/functions/durableScanWorkerControlV4/generatedReleaseContract.js",
  "base44/functions/durableScanWorkerControlV5/generatedReleaseContract.js",
  "base44/functions/durableScanWorkerControlV6/generatedReleaseContract.js",
  "base44/functions/durableScanWorkerControlV7/generatedReleaseContract.js",
  "base44/functions/durableScanWorkerControlV8/generatedReleaseContract.js",
  "base44/functions/getCustomerScanResultV2/generatedReleaseContract.js",
  "base44/functions/getCustomerScanResultV3/generatedReleaseContract.js",
  "base44/functions/getCustomerScanResultV4/generatedReleaseContract.js",
  "base44/functions/getCustomerScanResultV5/generatedReleaseContract.js",
  "base44/functions/getCustomerScanResultV6/generatedReleaseContract.js",
  "base44/functions/getCustomerScanResultV7/generatedReleaseContract.js",
  "base44/functions/getCustomerScanResultV8/generatedReleaseContract.js",
  // Not in RELEASE_FUNCTIONS and not reachable from the live app, but they must
  // not be able to ship a stale marker if they are ever republished.
  "base44/functions/aiReviewScan/generatedReleaseContract.js",
  "base44/functions/persistScanAuthority/generatedReleaseContract.js",
  "base44/functions/grokChat/generatedReleaseContract.js",
];

// These Base44 entry modules are the deployable module identities. Base44 has
// previously reused a compiled release when entry.ts stayed byte-identical while
// imported release-sensitive code changed. The canonical generator therefore
// rewrites a fingerprint marker in each release-sensitive entry whenever the
// frozen release fingerprint moves.
const ENTRY_IDENTITY_CONSUMERS = [
  "base44/functions/persistDurableScanAuthority/entry.ts",
  "base44/functions/persistLimitedScanResult/entry.ts",
  "base44/functions/startStandardScanJob/entry.ts",
  "base44/functions/durableScanWorkerControl/entry.ts",
  "base44/functions/getCustomerScanResult/entry.ts",
  "base44/functions/persistDurableScanAuthorityV2/entry.ts",
  "base44/functions/persistDurableScanAuthorityV3/entry.ts",
  "base44/functions/persistDurableScanAuthorityV4/entry.ts",
  "base44/functions/persistDurableScanAuthorityV5/entry.ts",
  "base44/functions/persistDurableScanAuthorityV6/entry.ts",
  "base44/functions/persistDurableScanAuthorityV7/entry.ts",
  "base44/functions/persistDurableScanAuthorityV8/entry.ts",
  "base44/functions/persistLimitedScanResultV2/entry.ts",
  "base44/functions/persistLimitedScanResultV3/entry.ts",
  "base44/functions/persistLimitedScanResultV4/entry.ts",
  "base44/functions/persistLimitedScanResultV5/entry.ts",
  "base44/functions/persistLimitedScanResultV6/entry.ts",
  "base44/functions/persistLimitedScanResultV7/entry.ts",
  "base44/functions/persistLimitedScanResultV8/entry.ts",
  "base44/functions/startStandardScanJobV2/entry.ts",
  "base44/functions/startStandardScanJobV3/entry.ts",
  "base44/functions/startStandardScanJobV4/entry.ts",
  "base44/functions/startStandardScanJobV5/entry.ts",
  "base44/functions/startStandardScanJobV6/entry.ts",
  "base44/functions/startStandardScanJobV7/entry.ts",
  "base44/functions/startStandardScanJobV8/entry.ts",
  "base44/functions/durableScanWorkerControlV2/entry.ts",
  "base44/functions/durableScanWorkerControlV3/entry.ts",
  "base44/functions/durableScanWorkerControlV4/entry.ts",
  "base44/functions/durableScanWorkerControlV5/entry.ts",
  "base44/functions/durableScanWorkerControlV6/entry.ts",
  "base44/functions/durableScanWorkerControlV7/entry.ts",
  "base44/functions/durableScanWorkerControlV8/entry.ts",
  "base44/functions/getCustomerScanResultV2/entry.ts",
  "base44/functions/getCustomerScanResultV3/entry.ts",
  "base44/functions/getCustomerScanResultV4/entry.ts",
  "base44/functions/getCustomerScanResultV5/entry.ts",
  "base44/functions/getCustomerScanResultV6/entry.ts",
  "base44/functions/getCustomerScanResultV7/entry.ts",
  "base44/functions/getCustomerScanResultV8/entry.ts",
  "base44/functions/ownerScanDebugControl/entry.ts",
  "base44/functions/aiReviewScan/entry.ts",
];


// Customer-facing Base44 functions published by deploy-base44-beta-site.sh.
// Their build identity is intentionally NOT part of the release fingerprint:
// it proves deployed package bytes for this exact source tree without changing
// the cross-runtime product contract.
const RELEASE_FUNCTIONS = [
  "startStandardScanJob",
  "durableScanWorkerControl",
  "persistDurableScanAuthority",
  "persistLimitedScanResult",
  "getCustomerScanResult",
  "createAccessCheckout",
  "stripeWebhook",
  "deleteCustomerScanData",
  "ownerScanDebugControl",
];

// The routed packages Base44 actually executes have their own package identity.
// V2-V7 started as copies of the canonical packages, but they have accumulated
// route-specific entry bytes and, in V7, substantial delivery code. Hashing the
// canonical directory for an alias therefore cannot prove which alias bytes are
// running. Every routed package is hashed independently; function.jsonc is part
// of that identity on purpose because the deployed function name is part of the
// package Base44 executes.
const ROUTE_CONTRACT = JSON.parse(
  fs.readFileSync(path.join(REPO_ROOT, "data/base44-function-routes.json"), "utf8"),
);
const ROUTE_ALIASES = ROUTE_CONTRACT.routes;
// Retired route generations remain resolvable for bounded historical recovery.
const HISTORICAL_ROUTE_ALIASES = Object.values(ROUTE_CONTRACT.historical_routes || {}).flatMap((routes) => Object.entries(routes));
const ALL_ROUTE_ALIASES = [...Object.entries(ROUTE_ALIASES), ...HISTORICAL_ROUTE_ALIASES];
const ROUTED_FUNCTIONS = [...new Set(ALL_ROUTE_ALIASES.map(([, alias]) => alias))];
const BUILD_ID_FUNCTIONS = new Set([...RELEASE_FUNCTIONS, ...ROUTED_FUNCTIONS]);
const FUNCTION_BUILD_ID_FILE = "generatedBuildId.js";

function functionBuildIdFiles(fnName) {
  const dir = path.join(ROOT, "base44/functions", fnName);
  if (!fs.existsSync(dir)) {
    if (ROOT === REPO_ROOT) {
      throw new Error(`Release Base44 function is missing: ${fnName}`);
    }
    return [];
  }
  return fs.readdirSync(dir)
    .sort()
    .filter((name) => name !== FUNCTION_BUILD_ID_FILE)
    .filter((name) => fs.statSync(path.join(dir, name)).isFile());
}

function computeFunctionBuildId(fnName) {
  const dir = path.join(ROOT, "base44/functions", fnName);
  const files = functionBuildIdFiles(fnName);
  if (files.length === 0) {
    throw new Error(`Release Base44 function has no source files: ${fnName}`);
  }
  const digest = crypto.createHash("sha256");
  for (const file of files) {
    digest.update(file);
    digest.update("\0");
    digest.update(fs.readFileSync(path.join(dir, file)));
    digest.update("\0");
  }
  return digest.digest("hex");
}

function functionBuildIdSource(fnName, buildId) {
  return [
    "// DO NOT EDIT. Generated by scripts/generate_release_contracts.mjs.",
    `// SHA-256 package identity for base44/functions/${fnName}.`,
    `// Every regular package file is covered except ${FUNCTION_BUILD_ID_FILE} itself.`,
    "",
    `export const FUNCTION_BUILD_ID = ${JSON.stringify(buildId)};`,
    "",
  ].join("\n");
}

function writeFunctionBuildIds({ check = false } = {}) {
  const drifted = [];
  for (const fnName of RELEASE_FUNCTIONS) {
    const dir = path.join(ROOT, "base44/functions", fnName);
    if (!fs.existsSync(dir)) {
      if (ROOT === REPO_ROOT) {
        throw new Error(`Release Base44 function is missing: ${fnName}`);
      }
      continue;
    }
    const relative = `base44/functions/${fnName}/${FUNCTION_BUILD_ID_FILE}`;
    const target = path.join(ROOT, relative);
    const buildId = computeFunctionBuildId(fnName);
    const expected = functionBuildIdSource(fnName, buildId);
    const current = fs.existsSync(target) ? fs.readFileSync(target, "utf8") : null;
    if (current === expected) continue;
    if (check) {
      drifted.push(`${relative} expected_build_id=${buildId}`);
      continue;
    }
    fs.writeFileSync(target, expected);
  }

  // Each routed alias identifies its own deployable package bytes. A V7-only
  // change must move the V7 build id even when the canonical package is
  // untouched; otherwise runtime verification can report a false green.
  for (const [, alias] of ALL_ROUTE_ALIASES) {
    const aliasDir = path.join(ROOT, "base44/functions", alias);
    if (!fs.existsSync(aliasDir)) continue;
    const relative = `base44/functions/${alias}/${FUNCTION_BUILD_ID_FILE}`;
    const target = path.join(ROOT, relative);
    const buildId = computeFunctionBuildId(alias);
    const expected = functionBuildIdSource(alias, buildId);
    const current = fs.existsSync(target) ? fs.readFileSync(target, "utf8") : null;
    if (current === expected) continue;
    if (check) {
      drifted.push(`${relative} expected_build_id=${buildId}`);
      continue;
    }
    fs.writeFileSync(target, expected);
  }
  return drifted;
}

function readJson(relative) {
  return JSON.parse(fs.readFileSync(path.join(ROOT, relative), "utf8"));
}

function contractSource() {
  const record = readJson(REVISION_RECORD);
  const input = readJson(CROSS_RUNTIME_INPUT);

  if (input.schema_version !== CROSS_RUNTIME_SCHEMA) {
    throw new Error(`Cross-runtime components must use ${CROSS_RUNTIME_SCHEMA}.`);
  }
  if (!/^[0-9a-f]{16}$/.test(String(record.fingerprint || ""))) {
    throw new Error("The recorded revision has no valid fingerprint.");
  }
  for (const [key, value] of Object.entries(input.components)) {
    if (record.component_versions?.[key] !== value) {
      throw new Error(
        `Cross-runtime component ${key} is not in the recorded fingerprint. `
        + "Re-run scanner-api/scripts/freeze_beta_revision.py first.",
      );
    }
  }

  // Sorted so the output is byte-identical for identical inputs.
  const components = Object.fromEntries(
    Object.entries(record.component_versions).sort(([a], [b]) => (a < b ? -1 : 1)),
  );

  return [
    "// DO NOT EDIT. Generated by scripts/generate_release_contracts.mjs from",
    `// ${REVISION_RECORD} and ${CROSS_RUNTIME_INPUT}.`,
    "// Run that generator after changing either input; CI fails on drift.",
    "",
    `export const RELEASE_CONTRACT_SCHEMA_VERSION = ${JSON.stringify(record.schema_version)};`,
    `export const RELEASE_FINGERPRINT = ${JSON.stringify(record.fingerprint)};`,
    `export const RELEASE_STATUS = ${JSON.stringify(record.status)};`,
    `export const RELEASE_COMPONENT_VERSIONS = Object.freeze(${JSON.stringify(components, null, 2)});`,
    "",
    "/** Exact-match gate. A missing or unexpected marker is never substituted. */",
    "export function releaseMarkersMatch(candidate) {",
    "  if (!candidate || typeof candidate !== \"object\") return false;",
    "  return String(candidate.beta_revision_fingerprint || \"\") === RELEASE_FINGERPRINT;",
    "}",
    "",
  ].join("\n");
}


function entryIdentitySource(source, fingerprint, relative) {
  const marker = /const BASE44_HANDLER_RELEASE_FINGERPRINT = "[0-9a-f]{16}";/;
  if (!marker.test(source)) {
    throw new Error(`Release-sensitive Base44 entry has no fingerprint marker: ${relative}`);
  }
  return source.replace(
    marker,
    `const BASE44_HANDLER_RELEASE_FINGERPRINT = "${fingerprint}";`,
  );
}

function main() {
  const mode = process.argv[2] || "";
  if (mode === "--list") {
    process.stdout.write(`${CONSUMERS.join("\n")}\n`);
    return 0;
  }
  if (mode === "--activation-id") {
    // The runtime activation marker a deployed handler must echo back. It is
    // read from the package's own entry.ts rather than restated here, so the
    // value verification demands and the value the handler returns cannot
    // drift apart. Unlike the build ID this is NOT resolved through the alias:
    // an alias carries its own marker precisely so the two routes are
    // distinguishable at runtime.
    const fnName = String(process.argv[3] || "");
    if (!RELEASE_FUNCTIONS.includes(fnName) && !ALL_ROUTE_ALIASES.some(([, alias]) => alias === fnName)) {
      process.stderr.write(`Unknown release Base44 function: ${fnName || "(missing)"}\n`);
      return 2;
    }
    const entry = path.join(ROOT, "base44/functions", fnName, "entry.ts");
    if (!fs.existsSync(entry)) {
      process.stderr.write(`Release Base44 function has no entry.ts: ${fnName}\n`);
      return 2;
    }
    const marker = fs.readFileSync(entry, "utf8")
      .match(/BASE44_RUNTIME_ACTIVATION_ID\s*=\s*"([A-Za-z0-9._-]{1,120})"/);
    if (!marker) {
      process.stderr.write(`Release Base44 function has no runtime activation marker: ${fnName}\n`);
      return 2;
    }
    process.stdout.write(`${marker[1]}\n`);
    return 0;
  }
  if (mode === "--build-id") {
    const requested = String(process.argv[3] || "");
    if (!BUILD_ID_FUNCTIONS.has(requested)) {
      process.stderr.write(`Unknown release Base44 function: ${requested || "(missing)"}\n`);
      return 2;
    }
    try {
      process.stdout.write(`${computeFunctionBuildId(requested)}\n`);
      return 0;
    } catch (error) {
      const message = String(error?.message || error || "");
      if (
        message.startsWith("Release Base44 function is missing:")
        || message.startsWith("Release Base44 function has no source files:")
      ) {
        process.stderr.write(`${message}\n`);
        return 2;
      }
      throw error;
    }
  }

  const source = contractSource();
  const fingerprint = String(readJson(REVISION_RECORD).fingerprint || "");
  const drifted = [];
  for (const relative of CONSUMERS) {
    const target = path.join(ROOT, relative);
    const current = fs.existsSync(target) ? fs.readFileSync(target, "utf8") : null;
    if (current === source) continue;
    if (mode === "--check") {
      drifted.push(relative);
      continue;
    }
    fs.mkdirSync(path.dirname(target), { recursive: true });
    fs.writeFileSync(target, source);
  }

  for (const relative of ENTRY_IDENTITY_CONSUMERS) {
    const target = path.join(ROOT, relative);
    if (!fs.existsSync(target)) {
      // Existing generator unit tests use isolated contract-only roots. The
      // real repository must always contain every deployable entry identity,
      // while isolated roots may omit entries unless that test is explicitly
      // exercising entry identity generation.
      if (ROOT === REPO_ROOT) {
        throw new Error(`Release-sensitive Base44 entry is missing: ${relative}`);
      }
      continue;
    }
    const current = fs.readFileSync(target, "utf8");
    const expected = entryIdentitySource(current, fingerprint, relative);
    if (current === expected) continue;
    if (mode === "--check") {
      drifted.push(relative);
      continue;
    }
    fs.writeFileSync(target, expected);
  }

  drifted.push(...writeFunctionBuildIds({ check: mode === "--check" }));

  if (mode === "--check" && drifted.length > 0) {
    process.stderr.write(
      `Generated release contracts are out of date:\n  ${drifted.join("\n  ")}\n`
      + "Run: node scripts/generate_release_contracts.mjs\n",
    );
    return 1;
  }
  return 0;
}

process.exit(main());
