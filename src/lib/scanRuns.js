...
export async function listScanRuns(projectId, limit = 3) {
  ...
  try {
    await currentOwner();
    const response = await base44.functions.invoke("getCustomerScanResultV7", {
      action: "list",
    ...
    const runs = result.runs.map((run) => ({
      ...
      path_prefix: String(run?.path_prefix || ""),
      scope_type: String(run?.scope_type || ""),
      parent_scan_id: String(run?.parent_scan_id || ""),
      requested_origin: String(run?.requested_origin || ""),
      requested_path_prefix: String(run?.requested_path_prefix || run?.path_prefix || ""),
      discovered_from: String(run?.discovered_from || ""),
...
export async function getScanRunWithFixList(scanRunId) {
  ...
  try {
    ...
    // getCustomerScanResultV7. Its two customer-visible FixItems are
