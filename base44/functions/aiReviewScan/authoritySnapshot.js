...
export function buildAuthoritySnapshot({ scan, review, identity, userId, now = new Date().toISOString() }) {
  ...
  return {
    ...
    scan: {
      ...
      // Mirrors persistDurableScanAuthority/authoritySnapshot.js. This builder
      // is legacy and unreachable, but its shape is round-tripped against the
      // shared reconstructor, so it cannot drift from the durable one.
