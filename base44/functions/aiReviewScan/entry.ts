...
const AI_REVIEW_VERSION = "aiReviewScan_v4_marketplace_lending_archetypes";
const SCORING_MODEL = "fixlist_archetype_playbooks_v4_marketplace_lending_archetypes";
const CORS_HEADERS = {
...
const LOW_VALUE_PATTERNS = ["/actualites/", "/news/", "/archive/", "/archives/", "/tag/", "/tags/", "/author/", "/feed/", "/rss/", "/page/", "?page=", "&page=", "?tag=", "&tag="];
...
const TRUST_PAGE_PATTERNS = ["/about", "/about-us", "/contact", "/privacy", "/privacy-policy", "/terms", "/terms-of-service", "/security", "/legal", "/mentions-legales", "/cgv", "/conditions"];
...
const ARCHETYPE_PLAYBOOKS = {
  finance_insurance_lead_gen: {
    ...
    keywords: ["assurance", "insurance", "pret", "prêt", "credit", "crédit", "loan", "loans", "lending", "lender", "mortgage", "finance", "banque", "bank", "taux", "emprunteur", "mutuelle", "devis", "simulation", "comparateur", "courtier", "broker", "hard money", "bridge loan", "fix and flip", "fix-and-flip", "rental", "dscr", "real estate investor", "private lending", "payoff", "document exchange", "apply now", "loan overview"],
    moneyPatterns: ["/devis", "/quote", "/simulation", "/simulateur", "/calcul", "/calculator", "/comparateur", "/compare", "/tarif", "/contact", "/souscription", "/assurance", "/credit", "/pret", "/loan", "/loans", "/loan-overview", "/apply", "/apply-now", "/request-a-payoff", "/document-exchange", "/locations", "/fix-and-flip", "/bridge", "/rental", "/dscr"],
    priorityPages: ["loan program pages", "application pages", "quote/contact forms", "calculator or rate pages", "location pages", "legal/trust pages", "methodology/review pages"],
    priorityIssues: ["indexability", "canonicalization", "trust pages", "schema", "broken lead paths", "form/application flow reliability", "template-level loan page issues"],
  ...
  booking_experiences_marketplace: {
    ...
    keywords: ["booking", "reservation", "réservation", "activity", "activities", "activite", "activité", "activités", "experience", "expérience", "experiences", "event", "tour", "destination", "billet", "ticket", "travel", "voyage", "stage", "pilotage", "pass", "atelier", "cadeau", "coffret", "loisir", "annonce", "voir", "funbooker", "domaine", "hotel", "chateau"],
    moneyPatterns: ["/booking", "/reservation", "/activity", "/activities", "/activite", "/activité", "/event", "/tour", "/destination", "/billet", "/ticket", "/stage", "/pilotage", "/pass", "/show", "/checkout", "/annonce", "/annonces", "/voir", "/cadeau", "/coffret", "/loisir"],
    ...
    priorityIssues: ["JavaScript rendering", "crawlable listing content", "booking route boundaries", "schema", "blocked listings", "duplicate templates", "missing trust signals", "template image-alt gaps on activity cards"],
    demote: ["old editorial posts", "tag archives", "low-value pagination", "one-off metadata on inactive listings"],
    ownerRule: "Rendering, schema, canonical, booking-flow, marketplace templates, and repeated listing-card fixes usually need your_web_person.",
  },
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const evidenceFixes = buildScannerEvidenceFindings({ body, pages, siteFingerprint });
    const pagePatternFixes = buildPagePatternFindings({ pages, siteFingerprint });
    const strategicFixes = buildStrategicFindings({ body, pages, websiteUrl, siteFingerprint, playbook });
    const canonicalFixes = prepareFixes([...rawFixes, ...evidenceFixes, ...pagePatternFixes, ...strategicFixes], siteFingerprint, body, playbook);
    ...
    return jsonResponse({ success: true, ai_provider: "scanner_fallback", ai_review_version: AI_REVIEW_VERSION, ai_review_warning: warning, ...fallbackPlan });
  } catch (error) {
...
function buildSiteFingerprint({ body, pages, websiteUrl }) {
  ...
  const pageText = safePages.slice(0, 220).map((page) => [page?.url, page?.final_url, page?.path, page?.title, page?.h1, page?.meta_description, page?.page_template_family, page?.estimated_page_intent, (page?.schema_types || []).join(" ")].join(" ")).join(" ").toLowerCase();
  ...
  const scores = Object.entries(ARCHETYPE_PLAYBOOKS).filter(([key]) => key !== "general").map(([key, playbook]) => ({
    key,
    score: playbook.keywords.reduce((total, keyword) => total + countIncludes(bodyText, keyword), 0) + playbook.moneyPatterns.reduce((total, pattern) => total + countIncludes(bodyText, pattern), 0) * 1.8 + archetypeBoost(key, bodyText, safePages),
  })).sort((a, b) => b.score - a.score);
  const primary = scores[0]?.score > 0 ? scores[0].key : "general";
  const secondary = scores[1]?.score > Math.max(3, scores[0]?.score * 0.6) ? scores[1].key : "";
  const confidence = scores[0]?.score > 0 ? Math.min(0.96, 0.45 + scores[0].score / Math.max(12, scores[0].score + (scores[1]?.score || 0))) : 0.35;
...
function archetypeBoost(key, text, pages) {
  const pathText = pages.slice(0, 220).map((page) => cleanPath(page?.url || page?.final_url || page?.path || "").toLowerCase()).join(" ");
  if (key === "booking_experiences_marketplace") {
    let score = 0;
    score += countIncludes(pathText, "/annonce/") * 8;
    score += countIncludes(pathText, "/voir") * 5;
    if (hasAny(text, ["funbooker", "activité", "activite", "cadeau", "coffret", "loisir", "reservation", "réservation"])) score += 35;
    return score;
  }
  if (key === "finance_insurance_lead_gen") {
    let score = 0;
    score += countIncludes(pathText, "/loans") * 10;
    score += countIncludes(pathText, "/loan") * 6;
    score += countIncludes(pathText, "/apply-now") * 18;
    score += countIncludes(pathText, "/request-a-payoff") * 18;
    score += countIncludes(pathText, "/document-exchange") * 14;
    score += countIncludes(pathText, "/locations") * 4;
    if (hasAny(text, ["centerstreetlending", "center street lending", "fix-and-flip", "fix and flip", "hard money", "bridge loan", "private lending", "real estate investor", "lending"])) score += 55;
    return score;
  }
  return 0;
}
...
function buildScannerEvidenceFindings({ body, pages, siteFingerprint }) {
  const evidencePages = dedupePages([...collectArrays([body.verified_failed_pages, body.technical_audit_summary?.verified_failed_pages, body.url_evidence_summary?.verified_failed_pages]), ...(Array.isArray(pages) ? pages.filter((page) => isFailedPage(page) || isBlockedAccessPage(page)) : [])]);
  const fixes = [];
  const byBucket = new Map();
  for (const page of evidencePages) {
    const bucket = statusBucketFromPage(page);
    if (!byBucket.has(bucket)) byBucket.set(bucket, []);
    byBucket.get(bucket).push(page);
  }
  for (const [bucket, group] of byBucket.entries()) {
    if (bucket === "429") {
      fixes.push(makeSyntheticFix({ rule: "rate_limited_page", category: "web_dev", priority: group.length >= 3 ? "high" : "medium", title: group.length >= 3 ? "Check pages blocked by rate limiting" : "Verify crawler access for a rate-limited page", explanation: "The scanner saw HTTP 429, bot protection, or a connection-verification response. This is crawler-access evidence, not proof that customers see a broken page.", why: "If legitimate crawlers cannot access important pages, search engines may miss them. Verify this in server, CDN, firewall, or bot-protection logs before changing page content.", recommendation: "Ask your web person to check server, CDN, firewall, and bot-protection logs for these URLs. Confirm whether Googlebot and normal users can access them, then adjust rate-limit rules only if legitimate access is blocked.", affectedPages: group.map(pageEvidenceUrl).filter(Boolean), difficulty: "developer", source: "scanner_verified_failed_pages:429", extra: evidenceExtra(group) }));
      continue;
    }
    ...
    fixes.push(makeSyntheticFix({
      ...
      title: isServer ? "Fix server errors found during the crawl" : "Fix confirmed broken URLs found during the crawl",
      explanation: isServer ? "The scanner found URLs that returned server errors during the crawl. These are technical availability problems, not copywriting tasks." : "The scanner found URLs that returned 404 or 410 during the crawl and included source evidence such as internal links, sitemap discovery, or linked failed URLs.",
      why: isServer ? "Server errors can prevent search engines and users from reaching important pages and can waste crawl budget." : "Broken internal links and confirmed failed URLs waste crawl budget and can send users or search engines into dead ends, especially when they are discovered from important pages.",
      recommendation: isServer ? "Ask your web person to inspect the failing URLs, server logs, and routing rules, then restore the page or redirect to the closest relevant live page." : "Ask your web person to either restore the missing URL, update the internal link that points to it, or add a 301 redirect to the closest relevant live page. Do not treat this as a meta title or content rewrite.",
  ...
  if (suspiciousArtifacts.length > 0) fixes.push(makeSyntheticFix({ rule: "suspicious_url_artifacts", category: "web_dev", priority: "low", title: "Review suspicious crawler URL artifacts separately from real broken pages", explanation: "The scanner saw URL-like artifacts that may come from encoded scripts, assets, or parser noise. They should not be mixed into confirmed broken pages without source evidence.", why: "Separating suspicious artifacts from confirmed URLs prevents FixList from telling customers to repair pages that may never exist as real routes.", recommendation: "Use the evidence list to confirm whether each artifact is a real internal link before adding redirects or content work.", affectedPages: suspiciousArtifacts.map((item) => cleanPath(item?.url || item?.path || item)).slice(0, 80), difficulty: "developer", source: "scanner_suspicious_url_artifacts" }));
  return fixes;
}

function buildPagePatternFindings({ pages }) {
  const safePages = Array.isArray(pages) ? pages : [];
  ...
  for (const page of safePages) {
    ...
    if (!url) continue;
    if (!cleanString(page?.canonical || page?.canonical_url)) addPageBucket(buckets, "canonical_missing", "canonical", family, page, "Add canonical URLs across templates", "The page does not expose a canonical URL.", "Canonical URLs help search engines consolidate duplicate and near-duplicate versions of a page.", "Ask your web person to add self-referencing canonicals to the shared template or affected pages.", "developer");
    const h1Count = Number(page?.h1_count || 0);
    if (h1Count === 0) addPageBucket(buckets, "missing_h1", "thin_content", family, page, "Fix missing H1 headings on templates", "The page has no main H1 heading.", "A clear H1 helps users and search engines understand the page topic.", "Add one clear H1 to the affected template or page.", "moderate");
    if (h1Count > 1) addPageBucket(buckets, "multiple_h1", "thin_content", family, page, "Use one main page heading", "The page has more than one H1 heading.", "Multiple H1s can make the page structure less clear.", "Keep one H1 as the main page heading and make the rest H2/H3 headings.", "easy");
    const missingAlt = Number(page?.image_missing_alt_count || page?.missing_alt_image_count || 0);
    if (missingAlt > 0) addPageBucket(buckets, "image_alt_text", "image_alt_text", family, page, "Batch image descriptions on templates", `${missingAlt} images missing alt text`, "Repeated image-alt gaps are usually a shared template or CMS pattern, especially on listing or detail pages.", "Fix one representative page/template first, then roll out the same rule across the affected group.", missingAlt >= 8 ? "developer" : "easy");
    if (!cleanString(page?.meta_description)) addPageBucket(buckets, "missing_meta_description", "meta_description", family, page, "Batch meta descriptions on templates", "The page is missing a meta description.", "Search descriptions can improve how pages appear in search results.", "Add a short description that explains the page and why someone should click.", "easy");
  }
  return Array.from(buckets.values()).map((bucket) => makeSyntheticFix({ rule: bucket.rule, category: bucket.category, priority: bucket.category === "canonical" ? "critical" : bucket.pages.length >= 8 ? "high" : "medium", title: bucket.pages.length > 1 ? bucket.title.replace("templates", `${bucket.familyLabel} templates`) : bucket.title.replace("templates", "the affected page"), explanation: bucket.pages.length > 1 ? "Several similar pages have the same template-level issue. Fix the shared template or pattern instead of creating one task per page." : bucket.explanation, why: bucket.pages.length > 1 ? "Large sites usually have template problems. Grouping keeps the FixList focused on the highest-impact patterns." : bucket.why, recommendation: bucket.recommendation, affectedPages: bucket.pages.map(pageEvidenceUrl), difficulty: bucket.pages.length >= 5 ? "developer" : bucket.difficulty, source: `page_pattern:${bucket.rule}:${bucket.family}`, extra: { current_value: bucket.currentValue, page_template_family: bucket.family } }));
}
function addPageBucket(buckets, rule, category, family, page, title, explanation, why, recommendation, difficulty) {
  const key = `${rule}|${family}`;
  if (!buckets.has(key)) buckets.set(key, { rule, category, family, familyLabel: familyLabel(family), pages: [], title, explanation, why, recommendation, difficulty, currentValue: explanation });
...
function buildStrategicFindings({ body, pages, websiteUrl, siteFingerprint, playbook }) {
  ...
  if (duplicateCasing.length > 0) fixes.push(makeSyntheticFix({ rule: "duplicate_route_casing", category: "canonical", priority: "medium", title: "Normalize duplicate URL casing", explanation: "The scanner found route variants that differ only by capitalization. These can split signals or create duplicate crawl paths.", why: "Consistent casing keeps canonical signals cleaner and avoids duplicate routing edge cases.", recommendation: "Ask your web person to canonicalize casing, redirect non-preferred casing, and make internal links use the preferred lowercase route.", affectedPages: duplicateCasing.map((item) => cleanPath(item?.url || item?.path || item)).slice(0, 80), difficulty: "developer", source: "scanner_duplicate_casing_routes" }));
  return fixes;
}
...
function normalizeFix(fix, index) {
  ...
  const developerOwned = needsDeveloperOwner({ ...fix, rule, category, difficulty, affected_pages: affected });
  ...
  return { ...fix, id, fix_id: id, rule, category, customer_category: fix?.customer_category || friendlyCategory(category), issue_title: title, title, plain_english_explanation: explanation, plain_english_summary: cleanString(fix?.plain_english_summary || fix?.plain_english_explanation || fix?.explanation) || explanation, why_it_matters: why, recommended_value: recommendation, ai_recommendation: recommendation, current_value: cleanString(fix?.current_value || fix?.current || (fix?.status_code ? `HTTP ${fix.status_code}` : "")), page_url: pageUrl, affected_pages: affected, source_pages: Array.isArray(fix?.source_pages) ? fix.source_pages.slice(0, 20) : [], link_text_samples: Array.isArray(fix?.link_text_samples) ? fix.link_text_samples.slice(0, 10) : [], url_confidence: fix?.url_confidence || "", url_suspicion_reasons: Array.isArray(fix?.url_suspicion_reasons) ? fix.url_suspicion_reasons.slice(0, 8) : [], priority: normalizePriority(fix?.priority), difficulty: developerOwned ? "developer" : difficulty, status: developerOwned ? "needs_developer" : fix?.status || (fix?.can_auto_fix ? "auto_fixed" : "needs_approval"), requires_developer: developerOwned || Boolean(fix?.requires_developer), requires_approval: developerOwned ? false : fix?.requires_approval !== false, can_auto_fix: Boolean(fix?.can_auto_fix) && !developerOwned, what_to_do: steps, what_to_do_steps: steps, fix_steps: steps, who_can_do_this: developerOwned ? "your_web_person" : normalizeOwner(fix?.who_can_do_this), estimated_time: cleanString(fix?.estimated_time || fix?.time_estimate) || defaultTime(developerOwned ? "developer" : difficulty), time_estimate: cleanString(fix?.time_estimate || fix?.estimated_time) || defaultTime(developerOwned ? "developer" : difficulty), confidence_score: typeof fix?.confidence_score === "number" ? fix.confidence_score : 88 };
}
function scoreFixForSite(fix, siteFingerprint, body, playbook) {
  ...
  const trustBoost = siteFingerprint.regulatory_sensitivity !== "standard" && ["content_trust", "semantic_schema", "crawl_index", "structural"].includes(defectClass) ? 12 : 0;
  ...
  return { ...fix, priority, page_type: pageValue.classification, page_template_family: fix.page_template_family || getTemplateFamily(pageUrl), page_value_score: pageValue.score, page_value_label: pageValue.label, primary_defect_class: defectClass, meta_rewrite_allowed: false, meta_regeneration_gate: "not_metadata_primary_gap", business_importance: pageValue.classification, evidence_confidence: evidenceConfidence, reach_score: reachScore, overall_priority_score: overall, site_fingerprint_vertical: siteFingerprint.primary_archetype, archetype_label: siteFingerprint.archetype_label, requires_developer: developerOwned || fix.requires_developer, difficulty: developerOwned ? "developer" : fix.difficulty, status: developerOwned ? "needs_developer" : fix.status, who_can_do_this: developerOwned ? "your_web_person" : fix.who_can_do_this };
}
...
function buildFallbackPlan({ body, pages, canonicalFixes, siteFingerprint, playbook, websiteUrl }) {
  ...
  const report = { health_score: healthScore, score: healthScore, overall_explanation: summary, health_grade: healthScore >= 90 ? "Excellent" : healthScore >= 80 ? "Good" : healthScore >= 65 ? "Needs work" : "Poor", what_is_working: working, top_concerns: topConcerns, quick_wins: quickWins, bigger_projects: biggerProjects, limitations: ["This scan is read-only and cannot confirm private analytics, paid search data, conversions, or server logs.", "HTTP 429 and connection-verification results need access-log confirmation before being treated as confirmed broken customer pages."], next_best_step: canonicalFixes[0]?.issue_title || "Review the first FixList item." };
  return { plain_english_summary: summary, website_health_report: report, health_explanation: summary, customer_summary: summary, top_recommended_actions: canonicalFixes.slice(0, 5), recommended_actions: canonicalFixes, cleaned_fixes: canonicalFixes, raw_fixes: canonicalFixes, fixes: canonicalFixes, findings: canonicalFixes, recommendations: canonicalFixes, competitor_insights: [], grouped_page_recommendations: groupPageRecommendations(canonicalFixes), ignored_low_value_pages: (pages || []).filter((page) => isLowValuePage(page?.url || page?.final_url)).slice(0, 30).map((page) => cleanPath(page?.url || page?.final_url || "")), positive_findings: working, ai_rewrites_applied: 0, crawled_pages: (pages || []).slice(0, MAX_PAGES_RETURNED), pages: (pages || []).slice(0, MAX_PAGES_RETURNED), health_score: healthScore, site_fingerprint: siteFingerprint, archetype_playbook: { label: playbook.label, priority_pages: playbook.priorityPages, priority_issues: playbook.priorityIssues, demote: playbook.demote, owner_rule: playbook.ownerRule }, technical_audit_summary: body.technical_audit_summary || null, screaming_frog_lite_enabled: Boolean(body.screaming_frog_lite_enabled), audit_profile: cleanString(body.scanner_profile || body.audit_profile || ""), scanner_version: body.scanner_version || body.version || "", advanced_scan_backend: body.advanced_scan_backend || body.technical_audit_summary?.advanced_scan_backend || "", deno_fallback_used: Boolean(body.deno_fallback_used || body.technical_audit_summary?.deno_fallback_used), website_url: websiteUrl, seo_score: healthScore, simple_summary: summary, scanned_pages: (pages || []).slice(0, MAX_PAGES_RETURNED), scan_summary: { health_score: healthScore, score: healthScore, pages_scanned: siteFingerprint.pages_crawled, plain_english_summary: summary, site_fingerprint: siteFingerprint }, scoring_model: SCORING_MODEL };
}
function makeSyntheticFix({ rule, category, priority, title, explanation, why, recommendation, affectedPages, difficulty, source, extra = {} }) {
  ...
  return { id, fix_id: id, type: "site_level", rule, category, customer_category: friendlyCategory(category), issue_title: title, title, plain_english_explanation: explanation, plain_english_summary: explanation, why_it_matters: why, current_value: extra.current_value || (rule.includes("broken") || rule.includes("404") || rule.includes("410") ? "Confirmed failed URL evidence from scanner" : "Detected from crawl evidence and site patterns."), recommended_value: recommendation, ai_recommendation: recommendation, priority, difficulty, status: difficulty === "developer" ? "needs_developer" : "needs_approval", can_auto_fix: false, requires_approval: difficulty !== "developer", requires_developer: difficulty === "developer", affected_pages: cleanAffected, page_url: page, confidence_score: extra.confidence_score || 92, what_to_do: steps, what_to_do_steps: steps, fix_steps: steps, who_can_do_this: defaultOwner(difficulty), estimated_time: defaultTime(difficulty), time_estimate: defaultTime(difficulty), source, ...extra };
}
...
function computeHealthScore(fixes, siteFingerprint) { let score = 92; for (const fix of fixes || []) { if (fix.priority === "critical") score -= 12; else if (fix.priority === "high") score -= 8; else if (fix.priority === "medium") score -= 4; else score -= 1; } if (siteFingerprint.pages_crawled === 0) score = Math.min(score, 86); return Math.max(35, Math.min(98, score)); }
function scorePageValue(url, siteFingerprint, body, playbook) { const path = cleanPath(url).toLowerCase(); const requested = cleanPath(body?.requested_path_prefix || body?.crawl_path_prefix || body?.path_prefix || "").toLowerCase(); let score = 35; if (path === "/" || path.endsWith("/index.html")) score += 35; if (requested && (path === requested || path === `${requested}/` || path === `${requested}/index.html`)) score += 35; if (isRouteBoundaryCandidate(path) || isInternalAppRoute(path)) score += 32; for (const pattern of playbook.moneyPatterns || []) if (path.includes(pattern)) { score += 24; break; } if (hasAny(path, ["contact", "devis", "quote", "simulation", "calculator", "comparateur", "booking", "reservation", "annonce", "voir", "cadeau", "coffret", "loisir", "pricing", "demo", "signup", "product", "products", "collection", "collections", "checkout", "loan", "loans", "apply", "payoff", "document-exchange", "locations", "fix-and-flip"])) score += 24; if (isLowValuePage(path)) score -= 35; const clamped = Math.max(0, Math.min(100, score)); const classification = isRouteBoundaryCandidate(path) || isInternalAppRoute(path) ? "internal_or_auth_route" : clamped >= 70 ? "money_page" : clamped <= 30 ? "low_value" : "standard"; return { score: clamped, classification, label: classification === "internal_or_auth_route" ? "Route-boundary candidate" : classification === "money_page" ? "Important business page" : classification === "low_value" ? "Lower-priority archive/tag page" : "Standard page" }; }
...
function inferCategory(rule, fix) { const text = `${rule} ${fix?.title || ""} ${fix?.issue_title || ""}`.toLowerCase(); if (hasAny(text, ["429", "blocked", "rate limit", "bot protection", "server", "500", "503"])) return "web_dev"; if (text.includes("schema") || text.includes("trust")) return "schema"; if (text.includes("canonical")) return "canonical"; if (text.includes("h1") || text.includes("heading")) return "thin_content"; if (text.includes("title")) return "meta_title"; if (text.includes("description") || text.includes("meta")) return "meta_description"; if (text.includes("alt") || text.includes("image")) return "image_alt_text"; if (text.includes("404") || text.includes("410") || text.includes("broken")) return "404_error"; if (text.includes("index") || text.includes("noindex")) return "indexability"; return "web_dev"; }
function defaultTitle(category, rule = "") { if (/404|410|broken/i.test(`${category} ${rule}`)) return "Fix confirmed broken URLs"; const titles = { meta_title: "Improve search titles", meta_description: "Improve search descriptions", duplicate_content: "Review duplicate or repeated pages", canonical: "Add canonical URLs", schema: "Improve trust and structured data", thin_content: "Improve page headings", "404_error": "Fix pages that are not loading", web_dev: "Review website setup", image_alt_text: "Add useful image descriptions", indexability: "Review indexability settings" }; return titles[category] || "Review this recommendation"; }
...
function needsDeveloperOwner(item = {}) { const affectedCount = Array.isArray(item.affected_pages) ? new Set(item.affected_pages.map(cleanPath)).size : 0; const value = `${item.rule || ""} ${item.category || ""} ${item.title || ""} ${item.issue_title || ""} ${item.reason || ""} ${item.recommendation || ""} ${item.recommended_value || ""} ${pickFirstNonEmptyArray([item.what_to_do_steps, item.what_to_do]).join(" ")} ${item.who_can_do_this || ""} ${item.primary_defect_class || ""}`.toLowerCase(); const repeatedTemplate = affectedCount >= 5 && /batch|repeated|template|similar pages|shared template|pattern|several similar pages|roll out/.test(value); if (repeatedTemplate) return true; if (item.requires_developer || item.difficulty === "developer" || item.status === "needs_developer" || value.includes("your_web_person")) return true; return /developer|web person|server-side|server side|ssr|pre-render|prerender|javascript|rendering|schema|structured data|canonical|redirect|server|firewall|bot protection|cloudflare|429|500|503|404|410|robots|noindex|crawlable html|view source|indexability|route-boundary|route boundary|checkout|login|account|dashboard|routing/.test(value); }
function detectBusinessModel(text, archetype) { if (hasAny(text, ["/annonce", "/voir", "booking", "reservation", "réservation", "availability", "calendar", "book now", "ticket", "stage", "pass", "cadeau", "loisir"])) return "booking_or_reservation"; if (hasAny(text, ["/loans", "/loan", "apply-now", "fix-and-flip", "hard money", "bridge loan", "lending", "request-a-payoff"])) return "regulated_or_trust_lead_generation"; if (hasAny(text, ["devis", "quote", "simulation", "simulateur", "calcul", "calculator", "comparateur", "compare"])) return "quote_or_comparison_lead_gen"; if (hasAny(text, ["cart", "panier", "checkout", "sku", "product", "produit", "add to cart", "shopify"])) return "catalog_or_ecommerce"; if (hasAny(text, ["login", "dashboard", "subscription", "billing", "admin"])) return "saas_or_member_app"; if (["finance_insurance_lead_gen", "utilities_comparison_lead_gen"].includes(archetype)) return "regulated_or_trust_lead_generation"; if (archetype === "booking_experiences_marketplace") return "booking_or_reservation"; if (archetype === "ecommerce_specialty_retail") return "catalog_or_ecommerce"; return "content_or_general_business"; }
...
function dedupeByFixId(fixes) { const seen = new Set(); const output = []; for (const fix of fixes || []) { const key = fix.fix_id || fix.id || `${fix.rule}|${fix.category}|${fix.page_template_family || ""}|${fix.page_url}|${(fix.affected_pages || []).join(",")}`; if (seen.has(key)) continue; seen.add(key); output.push(fix); } return output; }
