...
def annotation(**overrides):
    payload = {
        ...
        "state_claims": [
            {"value": "complete", "source_ref": "$.status"}
        ],
    }
...
def test_v8_diagnostic_scalars_are_not_authorized_claim_sources():
    payload = annotation(
        numeric_claims=[
            {
                "value": 2,
...
def test_numeric_provenance_rejects_integer_float_type_drift():
    payload = annotation(
        numeric_claims=[{"value": 73.0, "source_ref": "$.health_score"}]
    )
    result = verify_grounded_payload(payload, sealed_l2=sealed_l2())
    assert result.status == "rejected"
...
def test_state_provenance_rejects_integer_float_type_drift():
    payload = annotation(
        numeric_claims=[],
        state_claims=[{"value": 73.0, "source_ref": "$.health_score"}],
    )
    result = verify_grounded_payload(payload, sealed_l2=sealed_l2())
    assert result.status == "rejected"
...
def test_equal_python_numbers_with_different_json_types_are_a_deterministic_conflict():
    first = annotation(annotation_id="a", numeric_claims=[{"value": 73, "source_ref": "$.health_score"}])
    second = annotation(annotation_id="b", numeric_claims=[{"value": 73.0, "source_ref": "$.health_score"}])
    result = verify_grounded_payload(
        {"schema_version": "chat_answer_v2", "answer_id": "conflict", "annotations": [first, second]},
        sealed_l2=sealed_l2(),
    )
    assert result.status == "rejected"
