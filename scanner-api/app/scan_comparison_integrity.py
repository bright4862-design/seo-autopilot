...
def build_validated_customer_scan_comparison_presentation(comparison: dict[str, Any]) -> dict[str, Any]:
    ...
    # The v1 customer buckets cannot account for a current finding without a
    # reference identity. Do not silently drop it and present zero changes.
    # The optional reader will show comparison unavailable; the result remains readable.
    if comparison["summary"]["current_repairs_without_reference_fingerprint"]:
        raise ValueError("Current repairs cannot all be represented in the comparison")
