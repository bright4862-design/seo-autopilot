...
async def read_scan_run(client: httpx.AsyncClient, scan_id: str) -> dict[str, Any] | None:
    ...
    response = await invoke_function(client, "durableScanWorkerControlV6", envelope, timeout=20.0)
...
async def reconcile_stale_scans(client: httpx.AsyncClient) -> dict[str, Any]:
    ...
    response = await invoke_function(client, "durableScanWorkerControlV6", envelope, timeout=60.0)
    if response["status_code"] >= 300 or response["body"].get("success") is not True:
...
async def persist_limited_result(
    ...
    *,
    ...
    persisted = await invoke_function(client, "persistLimitedScanResultV6", envelope)
...
async def complete_authority(
    ...
    persisted = await invoke_function(client, "persistDurableScanAuthorityV6", envelope)
    emit(
        "scan_job_completion_phase",
        scan_id=scan_id,
        attempt_count=attempt_count,
