...
def extract_named_robots_evidence(page: dict) -> dict:
    ...
    if status_code is not None and (
        (status == "available" and not 200 <= status_code < 300)
        or (status == "missing" and status_code not in {404, 410})
    ):
        raise ValueError("Robots status code contradicts retained policy state")
    ...
    for crawler_id, spec in NAMED_CRAWLERS.items():
        ...
        if status == "missing" and value is False:
            raise ValueError("Missing robots file contradicts retained disallow directive")
