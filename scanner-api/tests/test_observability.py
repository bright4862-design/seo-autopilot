...
def test_release_marker_endpoints_are_consistent():
    ...
    assert health["archetype_classifier_version"] == "archetype_classifier_v5_business_representative_pages"
    assert health["beta_revision_fingerprint"] == "188651de922d349c"
...
def test_scan_endpoint_returns_beta_revision_fingerprint(monkeypatch, capsys):
    ...
    response = client.post("/scan", json={"website_url": "https://example.com"})
    assert response.status_code == 200
    assert response.json()["beta_revision_fingerprint"] == "188651de922d349c"
