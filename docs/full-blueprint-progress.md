...
- **B10 complete:** deterministic accepted-main-content analysis remains internal; fingerprints/shingles/raw copy are excluded from HTTP/authority/customer boundaries. Direct authenticated `/scan` regression `d9061f23edd0a0541b017f42297bf60a9c401560`, CI `35524582769`, proves B10/B11/B13/B15 private sentinels are absent from the serialized response.
...
Scan-isolation P1 RED `6f1e2cc3ae519bbb494cab599579758f9efe427a` / CI `35503985665`; fix `56a9362ceb9ec1b06b88e26a0fd7db8fff97e92c`; stale lane expectation corrected at `cbfd2b1f4a696c209dfab90e3981f42b0cbaa481`; exact CI `35504200573` passed; fresh follow-up `5749166024` reported no unresolved material issue.
...
### B22 — partial; signed preview source + completion privacy GREEN, fresh independent review still open

The signed preview source preserves exact trusted producer identity, verified-only evidence, impact-4/5 preference with two-finding cap, exactly one best verified fallback when no high-impact finding exists, and fixed controller-owned sufficient-coverage wording only when normalized coverage is explicitly sufficient. Arbitrary producer coverage text is excluded from the nested preview source and enclosing completion-HMAC Review.

Material RED/GREEN evidence:

- fallback/privacy RED `ac2e1f0e3f1326073a6a37593071c00244338b11` / CI `35519832761`; corrected executable `e7719996e5454f19d373dc04557792992590d834` / CI `35520663236`;
- enclosing completion-Review leak RED `db6432fcf7258e64736d7a3eabeadd28e4cb907f` / CI `35521744377`; positive-allowlist GREEN `4d1575d92c6a3092661bcd290b7d8ef4ae883066` / CI `35522106799`;
- authority fail-closed RED `28f58efbd773637bfada09da5b0a99b7fcc03fbc` / CI `35528067336` (`4 failed, 2042 passed, 18 skipped`); GREEN `4a9e0d4dec048291be7956bb225ab1635af30bbc` / CI `35528215150` (`115` root; `2046 passed, 18 skipped` scanner-api). `select_private_preview()` now returns no qualification unless `authority_verified is True`.
- malformed completion-coverage shape RED `60f52e8b304f65c89a30436a478ab75047ad3aad` / CI `35534328975`: scalar/list `coverage_assessment` values leaked two private sentinels (`2 failed, 2046 passed, 18 skipped`); the same RED run also failed generated client/release-contract artifact verification.
- malformed-shape GREEN `7d7dd19e7c77bdc03a93a081e7dab42d23cffd00` / exact-head CI `35537701598`: present non-mapping `coverage_assessment` now projects to `{}` before HMAC signing; absent stays absent; valid mappings retain only positive-allowlisted scanner-owned fields. Both CI jobs passed: root `115 passed`; scanner-api `2048 passed, 18 skipped`; generated release contracts/frontend gates passed; Stage-1 synthetic corpus 14/55 with `full_30_site_gate=not_assessed`; frozen revision `01ebe8e90df1e6bd`; scanner image `sha256:a4118fa8ab73ac94d873e4aa30e4d9c1ef0db13c3ae061da8893b584af38bfc2`.

The prior generated-client artifact failure did not persist on the corrected exact head. Detailed latest checkpoint: `docs/superpowers/plans/2026-09-20-stage3-completion-coverage-shape-fail-closed.md`.
...
RED `c713baca3f65da28bd52616960fdcb30e8f52d49` / CI `35514082243`; executable GREEN `23d28ddee804e9c20db4136f06aab879685d1942` / CI `35514416069`; stable head `4ee24691f46721ffe8112bb435f278e5dd810721` / CI `35514613656`; focused review `5750230877` found no unresolved material defect.
...
Operator-only hardening shares RED/GREEN `28f58ef...` / `4a9e0d4...`: `suppressed_findings` is projected only when `operator_authorized is True` literally. Truthy non-booleans fail closed.
...
Stage 3 is **not complete**. Latest executable GREEN is `7d7dd19e7c77bdc03a93a081e7dab42d23cffd00` with exact-head CI `35537701598`. The latest detailed checkpoint is `docs/superpowers/plans/2026-09-20-stage3-completion-coverage-shape-fail-closed.md`. The documentation/persisted ledger head must be certified separately by exact-head FixList CI.

Open gates before Stage 3 can close:

1. fresh independent review of the latest corrected B22/B24 shared authority/privacy boundary; skipped/failed automated reviews are not approval;
...
2. Refresh PR #303 comments/reviews and require one fresh independent review of the latest B22/B24 authority/privacy corrections; do not count a skipped review as approval.
...
Runtime note: GitHub `setup-node` requested Node 20 but resolved Node `20.20.2`; this is not exact Node 20.19.5 evidence. No production deployment, `main` merge, worker/admission mutation, production scan, provider connection, schema/RLS change, secret rotation, Premium enablement or Grok enablement occurred in the latest slice.
