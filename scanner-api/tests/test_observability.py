...
def test_release_marker_endpoints_are_consistent():
    ...
    assert health["archetype_classifier_version"] == "archetype_classifier_v7_html_route_app_distribution"
    assert health["beta_revision_fingerprint"] == "2dd346f90e64f94b"
...
def test_scan_endpoint_returns_beta_revision_fingerprint(monkeypatch, capsys):
    ...
    response = client.post("/scan", json={"website_url": "https://example.com"})
    assert response.status_code == 200
    assert response.json()["beta_revision_fingerprint"] == "2dd346f90e64f94b"
