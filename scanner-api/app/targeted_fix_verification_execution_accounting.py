...
TARGETED_FIX_VERIFICATION_EXECUTION_ACCOUNTING_VERSION = (
    "targeted_fix_verification_execution_accounting_v2_exact_request_observation_delta"
)
...
def validate_targeted_verification_execution_accounting(
    ...
    *,
    ...
    for observation in new_observations:
        ...
        if not isinstance(state, str) or state not in {"pass", "fail", "not_verified"}:
    ...
    # Every positive observation must account for one protected request or one
    # reuse. Refused/not-verified work can legitimately spend no request, but can
    # never create additional completed evidence or extra reuse outside the plan.
    observed_counts = {
        state: sum(item.get("state") == state for item in new_observations)
        for state in ("pass", "fail", "not_verified")
    }
    accounted_requests = consumed_delta + reused_delta
    positive_count = observed_counts["pass"] + observed_counts["fail"]
    if not positive_count <= accounted_requests <= expected_count:
        return _failure("targeted_execution_request_accounting_population_mismatch")
    if (
        observed_counts["pass"] != post_stats["passed"] - pre_stats["passed"]
        or observed_counts["fail"] != post_stats["failed"] - pre_stats["failed"]
        or observed_counts["not_verified"] != sum(
            post_stats[key] - pre_stats[key]
            for key in ("not_verified", "skipped", "exhausted")
        )
    ):
        return _failure("targeted_scheduler_observation_state_counts_mismatch")
