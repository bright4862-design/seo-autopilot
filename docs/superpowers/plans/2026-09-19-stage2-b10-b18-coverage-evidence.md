...
- **B13 local completeness/shell applicability:** `assess_local_entity_completeness` requires applicability + accepted evidence before evaluating local details. Missing regular hours fail only when hours are explicitly applicable; unknown applicability remains `not_verified`, and holiday hours/photos/sameAs/parent entities remain optional. `assess_local_shell` separately qualifies empty/Coming Soon shells against accepted HTML and contextual open/preopening status, and never treats failed/client-rendered evidence as a shell defect.
...
- **B17 page weight/CrUX:** `page_weight_evidence` keeps measured transfer bytes separate from decoded bytes and inline script/style bytes. If wire length was not measured, even a supplied numeric zero is reported as unknown. The optional CrUX adapter labels disconnected, unavailable, stale and connected evidence with provider/scope/time metadata; connected-but-empty metric payloads are `unavailable`, and HTML size is never substituted for field performance.
- **B18 optional GSC:** `optional_gsc_adapter` labels disconnected/unavailable/stale states without fabricating metrics. Connected observations retain only bounded page metrics/index state; arbitrary query payloads are not retained, and connected-but-empty usable evidence remains `unavailable`.

## Behavioral regression files

`scanner-api/tests/test_stage2_coverage_evidence.py` covers substantive near-duplicate clustering and clean passes; unverified text; sample-qualified money-page reachability; five-hub accounting/render-failure unknown semantics; local required/optional/applicability states; verified-versus-ambiguous NAP entity matching; contextual freshness; unknown wire length; CrUX states; and bounded GSC states.

`scanner-api/tests/test_stage2_local_shell_evidence.py` covers preopening Coming Soon context, open/Coming Soon contradiction, unverified status, accepted empty shells, rendering failure/client-rendering unknown states, and nonlocal applicability.
