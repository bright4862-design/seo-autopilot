...
if [ -f "$BASE44_SITE_DEPLOY" ]; then
  AUTH_LINE="$(grep -n 'fixlist_require_base44_owner' "$BASE44_SITE_DEPLOY" | head -1 | cut -d: -f1)"
  ...
  FUNCTIONS_LINE="$(grep -n 'deploy_functions "${FUNCTIONS\[@\]}"' "$BASE44_SITE_DEPLOY" | tail -1 | cut -d: -f1)"
  PROVEN_LINE="$(grep -n 'BASE44_RELEASE_ROUTES_ACTIVE_PRE_SITE' "$BASE44_SITE_DEPLOY" | head -1 | cut -d: -f1)"
  VERIFY_LINE="$(grep -n 'verify-base44-site.sh' "$BASE44_SITE_DEPLOY" | tail -1 | cut -d: -f1)"
  OWNER_CHECK_COUNT="$(grep -c 'fixlist_require_base44_owner' "$BASE44_SITE_DEPLOY" || true)"
  ...
  if [ -n "$PROVEN_LINE" ] && [ -n "$SITE_LINE" ] && [ "$PROVEN_LINE" -lt "$SITE_LINE" ]; then pass "Base44 release routes are proven live before the site is cut over"; else fail "Base44 site can publish in front of unproven release routes"; fi
  if [ "$OWNER_CHECK_COUNT" = "1" ] && ! grep -q '"$FIXLIST_BASE44_CLI" login' "$BASE44_SITE_DEPLOY"; then pass "Base44 site publish avoids interactive login"; else fail "Base44 site publish can stall on interactive login"; fi
