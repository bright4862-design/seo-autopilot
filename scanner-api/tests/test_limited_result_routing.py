...
def test_the_worker_routes_limited_coverage_to_the_limited_function():
    ...
    assert 'invoke_function(client, "persistLimitedScanResultV8"' in source
...
async def test_release_eligible_review_keeps_existing_authority_path(monkeypatch):
    ...
    async def invoke(_client, name, payload, *args, **kwargs):
        ...
        assert name == "persistDurableScanAuthorityV8"
        return {
            "status_code": 200,
            "body": {
                "success": True,
    ...
    assert [name for name, _payload in calls] == ["persistDurableScanAuthorityV8"]
