...
const STRUCTURAL_TEMPLATE_RULES = new Set([
  "client_rendering",
  "js_rendering",
  "canonical_missing",
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      try {
        ...
        writeScanDebug({ status: "running", stage: "ai_review_request_started", website_url: normalizedUrl, business_name: trimmedBusinessName, cms_platform: cmsPlatform, cms_name: cmsName, scan_mode: scanMode, requested_path_prefix: requestedPathPrefix, scanner: slimScannerData(scanData), ai_payload_summary: { crawled_pages_count: aiPayload.crawled_pages.length, raw_fixes_count: aiPayload.raw_fixes.length, crawl_policy_source: aiPayload.crawl_policy_source, url_evidence_preserved: Boolean(aiPayload.url_evidence_summary), business_priority_rules_enabled: true } });
        refreshDebugData();
        ...
        {debugOpen ? (
          <div className="mt-6 rounded-3xl border border-slate-200 bg-slate-50 p-4">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div><h3 className="font-bold text-slate-950">Scan debug</h3><p className="mt-1 text-xs text-slate-500">Compact debug summary for testing scanner, AI review, and final save.</p></div>
            ...
            <div className="mt-4 overflow-hidden rounded-2xl border border-slate-200 bg-slate-950"><pre className="max-h-[420px] overflow-auto p-4 text-xs leading-5 text-slate-100">{JSON.stringify(debugData, null, 2)}</pre></div>
          </div>
        ) : null}
          ...
          <div>
            ...
            <div className="mt-2 grid gap-3 md:grid-cols-3">
              {SCAN_MODES.map((mode) => {
                ...
                return <button key={mode.value} type="button" disabled={isLoading} onClick={() => setScanMode(mode.value)} className={`rounded-2xl border p-4 text-left transition ${active ? "border-indigo-500 bg-indigo-50 text-slate-950 ring-2 ring-indigo-100" : "border-slate-200 bg-white text-slate-700 hover:bg-slate-50"}`}><div className="flex items-center gap-2"><CheckCircle2 className={`h-4 w-4 ${active ? "text-indigo-600" : "text-slate-300"}`} /><span className="font-semibold">{mode.label}</span></div><p className="mt-2 text-xs leading-5 text-slate-500">{mode.description}</p></button>;
              })}
            </div>
          </div>
...
function buildAiReviewPayload({ scanData, businessName, websiteUrl, cmsPlatform, cmsName, cleanedKeywords, scanMode, requestedPathPrefix }) {
  return {
    ...
    business_priority_instruction: {
      ...
      high_priority_pages: ["homepage or scanned section landing page", "booking, checkout, product, listing, category, quote, calculator, comparison, pricing, contact, and trust pages", "pages with verified crawler provenance"],
      ...
      ownership_rule: "SSR, JavaScript rendering, schema templates, canonicals, redirects, firewall/bot protection, server errors, and crawlability belong to your_web_person.",
    ...
    output_requirements: {
      ...
      prioritize_money_pages_first: true,
      demote_low_value_archive_pages: true,
      group_repetitive_archive_issues: true,
      preserve_url_evidence_fields: true,
      do_not_overwrite_developer_owner_with_you: true,
    },
    crawled_pages: getPages(scanData).slice(0, 100),
    raw_fixes: getRecommendations(scanData).slice(0, 140),
    competitor_results: [],
    ...
    important_page_patterns: unique([requestedPathPrefix || "", ...MONEY_PAGE_PATTERNS]).slice(0, 50),
...
function mergeScanAndAiReview({ scanData, aiData, websiteUrl, businessName, cmsPlatform, cmsName, scanMode, requestedPathPrefix }) {
  ...
  const finalFixes = simplifyAndPrioritizeFixes((aiFixes.length > 0 ? aiFixes : scannerFixes), { requestedPathPrefix }).slice(0, 120);
  const pages = getPages(scanData).slice(0, 150);
  ...
  const topActions = buildTopActions({ aiData, finalFixes });
  const simpleSummary = aiData?.customer_summary || aiData?.plain_english_summary || aiData?.summary || aiData?.website_health_report?.overall_explanation || scanData?.scan_summary?.plain_english_summary || scanData?.scan_summary?.summary || buildFallbackSummary({ healthScore, pagesCrawled, finalFixes, cmsName });
  const technicalSummary = slimTechnicalSummary(scanData?.technical_audit_summary || {}, scanData);
  const crawlPolicy = scanData?.crawl_policy || technicalSummary.crawl_policy || {};
  const urlEvidenceSummary = scanData?.url_evidence_summary || technicalSummary.url_evidence_summary || {};
  ...
  return {
    ...
    top_recommended_actions: topActions.map(slimAction),
    ...
    scan_summary: slimScanSummary(aiData?.scan_summary || scanData?.scan_summary || scanData?.site_summary || {}, healthScore, pagesCrawled, finalFixes, scanData),
    site_summary: slimScanSummary(scanData?.site_summary || scanData?.scan_summary || {}, healthScore, pagesCrawled, finalFixes, scanData),
    technical_audit_summary: technicalSummary,
    website_health_report: slimHealthReport(aiData?.website_health_report || scanData?.website_health_report || {}),
    ...
    site_fingerprint: normalizeSiteFingerprint(aiData?.site_fingerprint || aiData?.scan_summary?.site_fingerprint || {}, pages),
    competitor_result: {},
    competitor_results: [],
...
function hydrateActionWithFix(action = {}, fix = null) {
  ...
  return {
    fix_id: action.fix_id || action.id || fix?.fix_id || fix?.id || "",
    title,
    reason: cleanString(action.reason || action.why_it_matters || action.plain_english_summary || fix?.why_it_matters || fix?.plain_english_explanation || ""),
    priority: normalizePriority(action.priority || fix?.priority),
    plain_english_summary: cleanString(action.plain_english_summary || fix?.plain_english_explanation || action.reason || ""),
    why_it_matters: cleanString(action.why_it_matters || action.reason || fix?.why_it_matters || ""),
    what_to_do_steps: steps.length > 0 ? steps : firstArray([fix?.what_to_do_steps, fix?.what_to_do]).slice(0, 5).map(String),
    who_can_do_this: developerOwned ? "Your web person" : normalizeOwner(action.who_can_do_this || fix?.who_can_do_this || "You"),
    time_estimate: action.time_estimate || action.estimated_time || fix?.time_estimate || fix?.estimated_time || "",
    affected_pages: firstArray([action.affected_pages, fix?.affected_pages]).slice(0, 20),
  };
}
...
function simplifyAndPrioritizeFixes(fixes, options = {}) {
  if (!Array.isArray(fixes)) return [];
  const enriched = fixes.map((fix) => applyBusinessPriorityRules(fix, options));
  const grouped = groupTemplateIssues(groupLowValueArchiveIssues(enriched));
  return grouped.sort((a, b) => businessSortScore(b, options) - businessSortScore(a, options));
}

function applyBusinessPriorityRules(fix = {}, options = {}) {
  const normalized = slimFix({ ...fix, priority: normalizePriority(fix?.priority), customer_category: fix?.customer_category || friendlyCustomerCategory(fix?.category), simple_next_step: fix?.simple_next_step || fix?.next_step || fix?.ai_recommendation || fix?.recommended_value || fix?.recommendation || fix?.suggested_fix || "Review this item and update the affected page." });
  ...
  const severe = isSevereIssue(normalized);
  const developerOwned = needsDeveloperOwner(normalized);

  let priority = normalizePriority(normalized.priority);
  let title = normalized.title;
  let issueTitle = normalized.issue_title;
  ...
  if (isImageAltTextIssue(normalized) && /metadata|meta descriptions|search titles/i.test(title)) {
    ...
    simpleNextStep = normalized.affected_pages?.length > 1 ? "Pick one listing as the example, add short useful alt text to important images, then apply the same image rule across the affected listings." : "Add short, useful alt text to the important images on this page.";
  }

  if (lowValue && isCosmeticRule(normalized) && !severe && !important) {
    priority = "low";
    title = title.includes("archive") ? title : `Low-priority archive cleanup: ${title}`;
    issueTitle = title;
    whyItMatters = "This is on a news, blog, tag, pagination, or archive page, so it is usually less important than fixing core service, product, booking, quote, calculator, listing, or contact pages first.";
    simpleNextStep = "Handle this later after the main business pages are cleaned up.";
  ...
  } else if (important && priority === "low") {
    priority = "medium";
  }
  ...
  return {
    ...
    requires_developer: developerOwned || normalized.requires_developer,
    requires_approval: developerOwned ? false : normalized.requires_approval,
    difficulty: developerOwned ? "developer" : normalized.difficulty,
    status: developerOwned ? "needs_developer" : normalized.status,
    who_can_do_this: developerOwned ? "your_web_person" : normalized.who_can_do_this,
    business_importance: normalized.business_importance && normalized.business_importance !== "standard" ? normalized.business_importance : important ? "important" : lowValue ? "low_value_archive" : "standard",
...
function groupTemplateIssues(fixes) {
  ...
  fixes.forEach((fix) => {
    const rule = fix.rule || fix.category || "";
    const family = fix.page_template_family || classifyTemplateFamily(fix.page_url || fix.affected_pages?.[0] || "");
    const affected = firstArray([fix.affected_pages]).length || 1;
    const shouldGroup = STRUCTURAL_TEMPLATE_RULES.has(rule) || (rule === "web_dev" && /render|javascript|schema|template/i.test(`${fix.title} ${fix.why_it_matters}`));
    if (!shouldGroup || affected > 1) {
      keep.push(fix);
      return;
    }

    const key = `${rule}|${family}|${normalizePriority(fix.priority)}`;
    const existing = groups.get(key) || {
      ...
      source_pages: [],
      link_text_samples: [],
      title: getTemplateGroupTitle(fix, family),
      issue_title: getTemplateGroupTitle(fix, family),
      plain_english_explanation: getTemplateGroupExplanation(fix),
      plain_english_summary: getTemplateGroupExplanation(fix),
      why_it_matters: getTemplateGroupWhy(fix),
      recommendation: getTemplateGroupRecommendation(fix),
      recommended_value: getTemplateGroupRecommendation(fix),
      simple_next_step: getTemplateGroupRecommendation(fix),
      difficulty: needsDeveloperOwner(fix) ? "developer" : fix.difficulty,
      status: needsDeveloperOwner(fix) ? "needs_developer" : fix.status,
      requires_developer: needsDeveloperOwner(fix) || fix.requires_developer,
      requires_approval: needsDeveloperOwner(fix) ? false : fix.requires_approval,
      who_can_do_this: needsDeveloperOwner(fix) ? "your_web_person" : fix.who_can_do_this,
      reach_score: Math.max(Number(fix.reach_score || 0), 60),
      overall_priority_score: Math.max(Number(fix.overall_priority_score || 0), fix.priority === "high" ? 78 : 55),
    };

    existing.affected_pages = unique([...existing.affected_pages, ...firstArray([fix.affected_pages]), fix.page_url].filter(Boolean).map(String)).slice(0, 120);
    existing.source_pages = unique([...firstArray([existing.source_pages]), ...firstArray([fix.source_pages])]).slice(0, 20);
    existing.link_text_samples = unique([...firstArray([existing.link_text_samples]), ...firstArray([fix.link_text_samples])]).slice(0, 10);
    existing.page_count = existing.affected_pages.length;
    groups.set(key, existing);
  });

  for (const group of groups.values()) {
    if ((group.affected_pages || []).length >= 3) keep.push(group);
    else keep.push(...fixes.filter((fix) => `${fix.rule || fix.category || ""}|${fix.page_template_family || classifyTemplateFamily(fix.page_url || fix.affected_pages?.[0] || "")}|${normalizePriority(fix.priority)}` === `${group.rule || group.category || ""}|${group.page_template_family || classifyTemplateFamily(group.page_url || group.affected_pages?.[0] || "")}|${normalizePriority(group.priority)}`));
  }

  return dedupeFixes(keep);
}

function groupLowValueArchiveIssues(fixes) {
  const groups = new Map();
  const keep = [];
  fixes.forEach((fix) => {
    const shouldGroup = fix.is_low_value_page && !fix.is_important_business_page && fix.priority === "low" && isCosmeticRule(fix);
    if (!shouldGroup) {
      keep.push(fix);
      return;
    }
    const key = `${fix.rule || fix.category || "archive_cleanup"}`;
    const existing = groups.get(key) || { ...fix, id: stableId(`archive_group_${key}`), fix_id: stableId(`archive_group_${key}`), title: getArchiveGroupTitle(fix), issue_title: getArchiveGroupTitle(fix), plain_english_explanation: "Several news, blog, tag, pagination, or archive pages have the same lower-priority SEO cleanup issue.", plain_english_summary: "Several news, blog, tag, pagination, or archive pages have the same lower-priority SEO cleanup issue.", why_it_matters: "These pages matter less than core service, quote, calculator, listing, booking, product, and contact pages. Grouping them keeps the FixList focused on business impact first.", recommendation: "Review these archive pages later as a batch, or leave them until higher-value pages are fixed.", recommended_value: "Review these archive pages later as a batch, or leave them until higher-value pages are fixed.", simple_next_step: "Fix your important pages first. Then decide whether these archive pages need cleanup at all.", affected_pages: [], page_url: "", current_value: "Grouped low-priority archive/news pages", estimated_time: "later", page_count: 0 };
    existing.affected_pages = unique([...existing.affected_pages, ...firstArray([fix.affected_pages]), fix.page_url].filter(Boolean).map(String)).slice(0, 100);
    existing.page_count = existing.affected_pages.length;
    groups.set(key, existing);
  });
  return dedupeFixes([...keep, ...Array.from(groups.values())]);
}
...
function getTemplateGroupExplanation(fix = {}) {
  if (/client|javascript|render/i.test(`${fix.rule} ${fix.title}`)) return "Several similar business-critical pages appear to rely on JavaScript for their main content. Treat this as one template rendering issue, not dozens of separate page edits.";
  return "Several similar pages have the same template-level issue. Fix the shared template or pattern instead of creating one task per page.";
}

function getTemplateGroupWhy(fix = {}) {
  if (/client|javascript|render/i.test(`${fix.rule} ${fix.title}`)) return "Search engines and AI crawlers may not see the booking, listing, or product content if it only appears after JavaScript runs.";
  return "Large sites usually have template problems. Grouping keeps the FixList focused on the highest-impact patterns.";
}

function getTemplateGroupRecommendation(fix = {}) {
  if (/client|javascript|render/i.test(`${fix.rule} ${fix.title}`)) return "Ask your web person to check whether the main booking/listing content is visible in page source. Add server-side rendering, pre-rendering, or crawlable fallback HTML for the affected template.";
  if (/schema/i.test(`${fix.rule} ${fix.title}`)) return "Ask your web person to add the right structured data once at the shared template level, then test a few affected pages.";
  return "Fix one representative page/template first, then roll out the same rule across the affected group.";
}
...
async function callBase44Function(functionName, payload) {
  ...
  return await Promise.race([callBase44FunctionWithoutTimeout(functionName, payload), new Promise((_, reject) => window.setTimeout(() => reject(new Error(`${functionName} did not return within ${Math.round(timeoutMs / 1000)} seconds. The scan may have timed out before saving results.`)), timeoutMs))]);
}
...
function buildCmsActionPlan(cmsPlatform, cmsName, fixes) {
  ...
  const cmsSteps = {
    wordpress: ["Open the page in WordPress.", "Update SEO fields in your SEO plugin.", "Improve headings, content, internal links, and image alt text.", "Use developer help for rendering, schema, redirects, or server issues."],
    shopify: ["Open the product, collection, blog post, or page in Shopify.", "Edit the search engine listing preview.", "Improve product/page copy, headings, internal links, and image alt text.", "Use developer help for theme rendering, schema, redirects, or app issues."],
    custom: ["Update simple content issues in the CMS editor.", "Send technical items such as rendering, redirects, schema, canonicals, and performance to a developer.", "Recheck the page after changes are published."],
  };
...
function slimScanRecord(record = {}) {
  ...
  return { id: record.id || `scan_${Date.now()}`, created_at: record.created_at || new Date().toISOString(), website_url: record.website_url || "", website_key: record.website_key || normalizeWebsiteKey(record.website_url || ""), requested_path_prefix: record.requested_path_prefix || "", business_name: record.business_name || "", cms_platform: record.cms_platform || "custom", cms_name: record.cms_name || "Custom / Not sure", scan_mode: record.scan_mode || "quick", health_score: Number(record.health_score || record.seo_score || 0), seo_score: Number(record.seo_score || record.health_score || 0), pages_crawled: Number(record.pages_crawled || pages.length || 0), pages_found: Number(record.pages_found || pages.length || 0), customer_summary: record.customer_summary || record.simple_summary || "", simple_summary: record.simple_summary || record.customer_summary || "", cms_action_plan: record.cms_action_plan || "", top_recommended_actions: firstArray([record.top_recommended_actions]).slice(0, 5).map((action) => hydrateActionWithFix(action, findSimilarFix(action, recommendations))), recommendations: recommendations.slice(0, 120).map(slimFix), fixes: recommendations.slice(0, 120).map(slimFix), findings: recommendations.slice(0, 120).map(slimFix), crawled_pages: pages.slice(0, 150).map(slimPage), pages: pages.slice(0, 150).map(slimPage), scanned_pages: pages.slice(0, 150).map(slimPage), scan_summary: slimScanSummary(record.scan_summary || {}, record.health_score, record.pages_crawled, recommendations, record), site_summary: slimScanSummary(record.site_summary || record.scan_summary || {}, record.health_score, record.pages_crawled, recommendations, record), technical_audit_summary: slimTechnicalSummary(record.technical_audit_summary || {}, record), website_health_report: slimHealthReport(record.website_health_report || {}), positive_findings: firstArray([record.positive_findings]).slice(0, 8).map(String), health_explanation: record.health_explanation || "", crawl_policy: record.crawl_policy || record.technical_audit_summary?.crawl_policy || {}, crawl_policy_source: record.crawl_policy_source || record.crawl_policy?.source || record.technical_audit_summary?.crawl_policy?.source || "", url_evidence_summary: record.url_evidence_summary || record.technical_audit_summary?.url_evidence_summary || {}, verified_failed_pages: getFirstNumber([record.verified_failed_pages, record.scan_summary?.verified_failed_pages, record.technical_audit_summary?.verified_failed_pages]), suspicious_url_artifacts: getFirstNumber([record.suspicious_url_artifacts, record.scan_summary?.suspicious_url_artifacts, record.technical_audit_summary?.suspicious_url_artifacts]), crawl_scope: record.crawl_scope || {}, sitemap_priority_summary: slimSitemapSummary(record.sitemap_priority_summary || {}), important_page_patterns: firstArray([record.important_page_patterns]).slice(0, 20), deprioritized_page_patterns: firstArray([record.deprioritized_page_patterns]).slice(0, 30), recommended_followup_scans: firstArray([record.recommended_followup_scans]).slice(0, 10), site_fingerprint: record.site_fingerprint || record.scan_summary?.site_fingerprint || {}, competitor_result: {}, competitor_results: [], competitor_opportunities: [], crawl_warnings: firstArray([record.crawl_warnings]).slice(0, 10), debug: record.debug || {}, raw: record.raw || {} };
}
...
function slimTechnicalSummary(summary = {}, source = {}) {
  ...
  return { ...sourceSummary, audit_profile: sourceSummary.audit_profile || source?.audit_profile || "", scanner_version: sourceSummary.scanner_version || source?.scanner_version || source?.version || "", screaming_frog_lite_enabled: Boolean(sourceSummary.screaming_frog_lite_enabled || source?.screaming_frog_lite_enabled), pages_checked: Number(sourceSummary.pages_checked || sourceSummary.pages_crawled || source?.pages_crawled || 0), pages_crawled: Number(sourceSummary.pages_crawled || source?.pages_crawled || 0), pages_found: Number(sourceSummary.pages_found || source?.pages_found || 0), failed_pages: Number(sourceSummary.failed_pages || 0), verified_failed_pages: Number(sourceSummary.verified_failed_pages || source?.verified_failed_pages || source?.scan_summary?.verified_failed_pages || 0), suspicious_url_artifacts: Number(sourceSummary.suspicious_url_artifacts || source?.suspicious_url_artifacts || source?.scan_summary?.suspicious_url_artifacts || 0), route_boundary_risk: sourceSummary.route_boundary_risk || "", duplicate_casing_routes: firstArray([sourceSummary.duplicate_casing_routes]).slice(0, 20), free_base44_subdomain: Boolean(sourceSummary.free_base44_subdomain), crawl_policy: sourceSummary.crawl_policy || source?.crawl_policy || {}, crawl_policy_source: source?.crawl_policy_source || sourceSummary.crawl_policy?.source || "", url_evidence_summary: sourceSummary.url_evidence_summary || source?.url_evidence_summary || {}, crawl_warnings: firstArray([sourceSummary.crawl_warnings, source?.crawl_warnings]).slice(0, 20) };
}

function slimScanSummary(summary = {}, healthScore = 0, pagesCrawled = 0, fixes = [], source = {}) {
  return { ...summary, score: Number(summary.score || summary.health_score || healthScore || 0), health_score: Number(summary.health_score || summary.score || healthScore || 0), status_label: summary.status_label || scoreLabel(healthScore), plain_english_summary: summary.plain_english_summary || summary.summary || "", pages_scanned: Number(summary.pages_scanned || pagesCrawled || 0), pages_failed: Number(summary.pages_failed || source?.technical_audit_summary?.failed_pages || 0), verified_failed_pages: Number(summary.verified_failed_pages || source?.verified_failed_pages || source?.technical_audit_summary?.verified_failed_pages || 0), suspicious_url_artifacts: Number(summary.suspicious_url_artifacts || source?.suspicious_url_artifacts || source?.technical_audit_summary?.suspicious_url_artifacts || 0), crawl_policy_source: summary.crawl_policy_source || source?.crawl_policy_source || source?.crawl_policy?.source || "", high_priority_count: fixes.filter((fix) => ["critical", "high"].includes(fix.priority)).length, technical_issue_count: fixes.length };
}

function slimHealthReport(report = {}) {
  return { health_score: Number(report.health_score || report.score || 0), health_grade: report.health_grade || report.grade || "", overall_explanation: report.overall_explanation || report.summary || "", what_is_working: firstArray([report.what_is_working]).slice(0, 5), top_concerns: firstArray([report.top_concerns]).slice(0, 5), quick_wins: firstArray([report.quick_wins]).slice(0, 5), bigger_projects: firstArray([report.bigger_projects]).slice(0, 5), limitations: firstArray([report.limitations]).slice(0, 5), next_best_step: report.next_best_step || "" };
}

function slimSitemapSummary(summary = {}) {
  return { ...summary, sitemap_urls_found: Number(summary.sitemap_urls_found || 0), prioritized_urls_count: Number(summary.prioritized_urls_count || 0), examples: firstArray([summary.examples]).slice(0, 10) };
}
...
function normalizeSiteFingerprint(fingerprint = {}, pages = []) {
  ...
  if (bookingHits >= 5 && (!fingerprint.vertical || fingerprint.vertical === "ecommerce")) return { ...fingerprint, vertical: "travel_booking", vertical_label: "booking / experiences marketplace", business_model: "booking_or_reservation", vertical_confidence: Math.max(Number(fingerprint.vertical_confidence || 0), 0.85) };
  return fingerprint;
}

function buildFallbackSummary({ healthScore, pagesCrawled, finalFixes, cmsName }) {
  const label = scoreLabel(healthScore);
  const high = finalFixes.filter((fix) => ["critical", "high"].includes(fix.priority)).length;
  return `Your website health is currently rated as ${label}. FixList scanned ${pagesCrawled || 0} pages and found ${finalFixes.length} grouped recommendation${finalFixes.length === 1 ? "" : "s"}. ${high > 0 ? `${high} item${high === 1 ? "" : "s"} should be reviewed first.` : "Start with the first recommended fix."} CMS selected: ${cmsName}.`;
}

function getRecommendations(source = {}) {
  return firstArray([source?.recommendations, source?.cleaned_fixes, source?.fixes, source?.findings, source?.raw_fixes, source?.grouped_findings, source?.issues]);
}

function getPages(source = {}) {
  return firstArray([source?.crawled_pages, source?.pages, source?.scanned_pages, source?.crawl_pages]);
}

function getHealthScore(source = {}) {
  return getFirstNumber([source?.health_score, source?.seo_score, source?.scan_summary?.health_score, source?.scan_summary?.score, source?.website_health_report?.health_score, source?.website_health_report?.score]);
}

function businessSortScore(fix = {}, options = {}) {
  let score = Number(fix.overall_priority_score || 0);
  const priorityBonus = { critical: 1000, high: 800, medium: 500, low: 100 }[normalizePriority(fix.priority)] || 300;
  score += priorityBonus;
  if (fix.requires_developer && /client|render|javascript|schema|canonical|server|firewall|blocked|429/i.test(`${fix.rule} ${fix.title}`)) score += 130;
  if (fix.business_importance === "money_page" || fix.is_important_business_page) score += 120;
  if (isImportantBusinessPage(fix.page_url || fix.affected_pages?.[0] || "", options)) score += 100;
  if (fix.is_low_value_page || isLowValuePage(fix.page_url || fix.affected_pages?.[0] || "")) score -= 180;
  if (fix.url_confidence === "crawler_artifact") score -= 300;
  if ((fix.affected_pages || []).length > 3) score += Math.min(90, fix.affected_pages.length * 4);
  return score;
}

function needsDeveloperOwner(item = {}) {
  const value = `${item.rule || ""} ${item.category || ""} ${item.title || ""} ${item.issue_title || ""} ${item.reason || ""} ${item.recommendation || ""} ${item.recommended_value || ""} ${firstArray([item.what_to_do_steps, item.what_to_do]).join(" ")} ${item.who_can_do_this || ""}`.toLowerCase();
  if (item.requires_developer || item.difficulty === "developer" || item.status === "needs_developer" || value.includes("your_web_person")) return true;
  return /developer|web person|server-side|server side|ssr|pre-render|prerender|javascript|rendering|schema|structured data|canonical|redirect|server|firewall|bot protection|cloudflare|429|500|503|robots|noindex|crawlable html|view source/.test(value);
}

function normalizeOwner(value) {
  const owner = String(value || "").toLowerCase();
  if (owner.includes("web") || owner.includes("developer")) return "Your web person";
  if (owner === "your_web_person") return "Your web person";
  return "You";
}

function isCosmeticRule(fix = {}) {
  const value = `${fix.rule || ""} ${fix.category || ""} ${fix.title || ""}`.toLowerCase();
  return /meta|title|description|thin_content|duplicate|image_alt|alt text|h1/.test(value);
}

function isSearchAppearanceIssue(fix = {}) {
  return /meta|title|description|duplicate|search/i.test(`${fix.rule || ""} ${fix.category || ""} ${fix.title || ""}`);
}

function isImageAltTextIssue(fix = {}) {
  return /image_alt_text|image alt|alt text|missing alt|image description/i.test(`${fix.rule || ""} ${fix.category || ""} ${fix.title || ""} ${fix.issue_title || ""}`);
}

function isSevereIssue(fix = {}) {
  return /404|410|429|500|503|blocked|server_error|redirect_loop|noindex|canonical|client_rendering|javascript|schema/.test(`${fix.rule || ""} ${fix.category || ""} ${fix.title || ""} ${fix.current_value || ""}`.toLowerCase());
}
...
function isImportantBusinessPage(url = "", options = {}) {
  ...
  const requested = String(options.requestedPathPrefix || "").toLowerCase();
  if (requested && (path === requested || path === `${requested}/` || path.endsWith(`${requested}/index.html`))) return true;
  return MONEY_PAGE_PATTERNS.some((pattern) => path.includes(pattern));
}

function classifyTemplateFamily(url = "") {
  const path = String(url || "").toLowerCase();
  if (isLowValuePage(path)) return "archive";
  if (/checkout|booking|reservation|ticket_order|gift_voucher/.test(path)) return "booking_or_checkout";
  if (/listing|show|category|categorie|collection|pass|ticket|stage|pilotage/.test(path)) return "category_listing";
  if (/product|produit|\/p\//.test(path)) return "product_detail";
  if (/simulation|simulateur|calcul|calculator|comparateur|devis|quote|pricing|demo/.test(path)) return "conversion";
  if (/contact/.test(path)) return "contact";
  if (/faq|question/.test(path)) return "qa";
  if (/guide|blog/.test(path)) return "guide";
  if (/privacy|terms|legal|mentions-legales|security/.test(path)) return "legal_info";
  return "standard";
}
