...
def test_worker_uses_only_signed_hosted_boundaries():
    ...
    assert 'invoke_function(client, "persistDurableScanAuthorityV3", envelope)' in source
    assert 'invoke_function(client, "durableScanWorkerControlV3", envelope' in source
...
def test_durable_worker_stamps_python_backend_authority_markers():
    ...
    assert "await_hard_deadline" in durable
    assert "WORKER_COMPLETION_WALL_TIMEOUT_SECONDS" in durable
    assert "asyncio.wait_for(\n                complete_authority" not in durable
