...
const VERSION = "runAdvancedScan_v15_route_boundary_candidates";
...
const USER_AGENT = "Mozilla/5.0 (compatible; FixListBot/1.5; +https://base44.app)";
...
const INTERNAL_ROUTE_PATTERNS = ["/admin", "/developer", "/assistant", "/billing", "/login", "/register", "/forgot-password", "/reset-password", "/dashboard", "/issues", "/reports", "/crawl-status", "/metadata", "/canonicals", "/redirects", "/js-rendering", "/competitors"];
const ROUTE_BOUNDARY_CANDIDATE_SEGMENTS = ["/login", "/register", "/forgot-password", "/reset-password", "/account", "/my-account", "/dashboard", "/admin", "/billing", "/cart", "/checkout"];
const HARD_SKIP_SEGMENTS = ["/logout", "/search", "/feed", "/rss", "/wp-admin", "/wp-json", "/preview"];
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const rawFindings = buildFindings({ pages: crawlResult.pages, websiteUrl });
    ...
    return jsonResponse({
      ...
      scanner_profile: "screaming_frog_lite_route_boundary_candidates_v15",
  ...
  while (queue.length > 0 && pages.length < budget.max_pages && Date.now() - startedAt < budget.crawl_timeout_ms) {
    ...
    if (fetched.text && isHtmlContent(fetched.content_type) && fetched.status >= 200 && fetched.status < 400) {
      ...
      for (const link of links) {
        ...
        if (shouldCrawlUrl(link.href, start, normalizedPrefix) && !seen.has(canonicalizeUrl(link.href))) enqueueUrl({ queue, discoveryMap, url: link.href, discoveredFrom: "internal_link", sourcePage: fetched.final_url || clean, linkText: link.text, policy });
      }
    }
  }

  return { pages, pages_found: Math.max(seen.size + queue.length, pages.length), queued_remaining: queue.length, warnings, crawl_policy: policy };
}
...
- skip_patterns: low-value crawl-last advice only. Never use skip_patterns to hide route-boundary candidates.
- Never put homepage, contact, about, privacy, terms, product/category/listing, quote, booking, checkout, login, account, dashboard, simulation, calculator, demo, or pricing paths in skip_patterns.
...
function summarizeHomeFingerprint(html) {
  ...
  return { has_root_div: /id=["'](root|app|__next|__nuxt)["']/i.test(h), module_scripts: /<script[^>]+type=["']module["']/i.test(h), generator: (h.match(/<meta[^>]+name=["']generator["'][^>]+content=["']([^"']+)["']/i)?.[1] || "").slice(0, 80), body_word_count: stripHtml(h).split(/\s+/).filter(Boolean).length, markers: markers.filter((marker) => lower.includes(marker)).slice(0, 12) };
}
...
function sanitizePolicy(raw) {
  ...
  const skipPatterns = cleanList(parsed.skip_patterns).filter((pattern) => !isRouteBoundaryCandidate(pattern) && !matchesAnyPattern(pattern, ["/", "/contact", "/about", "/privacy", "/terms", "/security", "/product", "/products", "/category", "/listing", "/login", "/account", "/cart", "/checkout", "/dashboard", "/devis", "/quote", "/booking", "/reservation", "/simulation", "/calculator", "/pricing", "/demo"]));
...
function computeQueuePriority({ path, discovery, policy }) {
  ...
  const routeBoundary = isRouteBoundaryCandidate(clean);
  let score = 40;
  ...
  if (routeBoundary) score += sources.includes("sitemap") ? 8 : 16;
  if (family === "guide" || family === "qa") score += 8;
  if (isLowValuePage(clean)) score -= 25;
  if (ARTIFACT_SEGMENT_RE.test(clean)) score -= 60;
  if (policy) {
    if (matchesAnyPattern(clean, policy.priority_boost_patterns)) score += routeBoundary ? 8 : 35;
    if (matchesAnyPattern(clean, policy.priority_deprioritize_patterns)) score -= routeBoundary ? 10 : 35;
    if (matchesAnyPattern(clean, policy.skip_patterns) && !routeBoundary) score -= 45;
  }
  if (routeBoundary && !sources.includes("seed")) score = Math.min(score, 78);
...
function emptyDiscovery(source) { return { discovered_from: source ? [source] : [], source_pages: [], link_text_samples: [] }; }
...
async function loadSitemapUrls(origin, limit) {
  ...
  if (robots?.text) for (const match of robots.text.match(/^sitemap:\s*(.+)$/gim) || []) sitemapCandidates.push(match.replace(/^sitemap:\s*/i, "").trim());
...
function shouldCrawlUrl(url, start, pathPrefix) {
  try {
    ...
    if (HARD_SKIP_SEGMENTS.some((segment) => lower === segment || lower.startsWith(`${segment}/`))) return false;
...
function extractPageFromHtml({ requestedUrl, fetched, discovery }) {
  ...
  const routeBoundaryCandidate = isRouteBoundaryCandidate(path);

  return { url: requestedUrl, final_url: finalUrl, path, discovered_from: discovery?.discovered_from || [], source_pages: discovery?.source_pages || [], link_text_samples: discovery?.link_text_samples || [], url_confidence: evidence.confidence, url_suspicion_reasons: evidence.reasons, route_boundary_candidate: routeBoundaryCandidate, route_boundary_type: routeBoundaryCandidate ? classifyRouteBoundaryType(path) : "", status_code: status, fetch_error: fetched.error || "", content_type: fetched.content_type || "", title, title_length: title.length, meta_description: metaDescription, meta_description_length: metaDescription.length, robots, viewport, canonical, canonical_status: canonical ? (sameCanonicalPath(finalUrl, canonical) ? "self_or_equivalent" : "canonical_to_different_url") : "missing", robots_indexability_status: indexable ? "indexable" : "not_indexable", h1: h1s[0] || "", h1_count: h1s.length, h2_count: h2s.length, word_count: wordCount, image_count: imageStats.total, image_missing_alt_count: imageStats.missingAlt, internal_link_count: linkStats.internal, external_link_count: linkStats.external, schema_types: schemaTypes, has_schema: schemaTypes.length > 0, client_rendering_suspected: clientRenderingSuspected, page_template_family: pageTemplateFamily, estimated_page_intent: estimatedPageIntent, conversion_signals: extractSignals(path, visibleText, ["contact", "quote", "devis", "demo", "pricing", "booking", "reservation", "checkout", "buy", "cart", "appointment"]), trust_signals: extractSignals(path, visibleText, ["privacy", "terms", "about", "contact", "address", "phone", "security", "reviews", "testimonials", "legal"]), indexable };
}
...
function buildFindings({ pages, websiteUrl }) {
  ...
  if (hostname.endsWith(".base44.app")) findings.push(createFinding({ rule: "free_base44_subdomain", category: "indexability", priority: "high", title: "Move production SEO to a custom domain", pageUrl: "/", affectedPages: ["/"], explanation: "The site is on a Base44 subdomain. It can be crawled, but a branded custom domain is a stronger long-term SEO and trust setup.", recommendation: "Connect a branded custom domain before treating this as the permanent production SEO home.", difficulty: "developer" }));
  ...
  for (const page of pages) {
    ...
    if (!page.canonical && page.indexable && !page.route_boundary_candidate && page.estimated_page_intent !== "internal_or_auth") findings.push(createFinding({ rule: "canonical_missing", category: "canonical", priority: page.estimated_page_intent === "money_or_conversion" ? "high" : "medium", title: "Add a canonical URL", pageUrl: path, affectedPages: [path], explanation: "The page does not expose a canonical URL.", recommendation: "Add a self-referencing canonical or point to the preferred version of this page.", difficulty: "developer" }));
...
function groupFindings(findings) {
  ...
  for (const finding of findings) {
    ...
    if (!groupKey) { direct.push(finding); continue; }
  ...
  for (const [key, bucket] of candidateGroups.entries()) {
    ...
    if (affected.length < 3 && !key.startsWith("suspicious|")) { direct.push(...bucket); continue; }
    ...
    const routeBoundary = key.startsWith("route_boundary|");
    const title = routeBoundary ? "Group route-boundary exposure checks" : suspicious ? "Verify suspicious URLs before calling them broken" : failure ? groupFailureTitle(sample, family) : groupTemplateTitle(sample, family);
    const explanation = routeBoundary ? "Several checkout, login, cart, account, dashboard, or app/private routes appear crawlable or indexable. Treat this as one route-boundary issue." : suspicious ? "Several failed URLs look encoded, malformed, or crawler-generated. They need source-link proof before being treated as real broken links." : failure ? "Several similar pages are failing or blocked in the same way. Treat this as one shared crawlability/template problem." : "Several similar pages have the same template-level issue. Fix the shared template or pattern instead of creating one task per page.";
    const recommendation = routeBoundary ? "Ask your web person to require login or add noindex for private/app/checkout/account routes while keeping true public pages crawlable and indexable." : suspicious ? "Check the source_pages and link_text_samples first. If no real internal link or sitemap source exists, ignore these as crawler artifacts or fix the extraction pattern." : failure ? "Check shared redirects, firewall/bot protection, server rules, or templates causing these failures, then rescan." : "Fix one representative page/template first, then roll out the same rule across the affected group.";
    direct.push({ ...sample, id: stableId(`group|${key}`), fix_id: stableId(`group|${key}`), page_url: "", issue_title: title, title, plain_english_explanation: explanation, plain_english_summary: explanation, why_it_matters: routeBoundary ? "Private or checkout/app routes in search can create duplicate, low-trust, or sensitive public pages and can confuse crawlers about what should rank." : explanation, recommended_value: recommendation, ai_recommendation: recommendation, priority: routeBoundary ? "critical" : suspicious ? "low" : failure ? "high" : sample.priority === "high" ? "medium" : sample.priority, difficulty: routeBoundary || failure ? "developer" : suspicious ? "moderate" : sample.difficulty, requires_developer: routeBoundary || failure || sample.requires_developer, who_can_do_this: routeBoundary || failure ? "your_web_person" : sample.who_can_do_this, affected_pages: affected.slice(0, 150), source_pages: Array.from(new Set(bucket.flatMap((finding) => finding.source_pages || []))).slice(0, 20), link_text_samples: Array.from(new Set(bucket.flatMap((finding) => finding.link_text_samples || []))).slice(0, 10), page_count: affected.length, source: routeBoundary ? "route_boundary_evidence" : suspicious ? "url_evidence_layer" : sample.source });
  }
...
function groupingKey(finding) {
  ...
  if (finding.rule === "route_boundary_candidate_indexable" || finding.rule === "internal_route_indexable") return `route_boundary|${family}`;
  ...
  if (["image_alt_text", "missing_meta_description", "missing_h1", "schema", "client_rendering"].includes(finding.rule)) return `template|${finding.rule}|${family}`;
...
function createFinding({ rule, category, priority, title, pageUrl, affectedPages, currentValue = "", explanation, recommendation, difficulty = "moderate", source = "screaming_frog_lite" }) {
  ...
  const steps = difficulty === "developer" ? ["Send this item to your web person.", "Ask them to review the affected URL, source page, or shared template.", "Apply the safest fix and publish it.", "Run FixList again to confirm it is resolved."] : ["Review the affected page, source page, or template.", recommendation, "Save and publish the change if needed.", "Run FixList again after publishing."];
  return { id, fix_id: id, type: "site_level", rule, category, customer_category: friendlyCategory(category), issue_title: title, title, page_url: cleanPath(pageUrl || "/"), affected_pages: Array.from(new Set((affectedPages || [pageUrl || "/"]).map((item) => cleanPath(item)).filter(Boolean))), plain_english_explanation: explanation, plain_english_summary: explanation, why_it_matters: explanation, current_value: currentValue, recommended_value: recommendation, ai_recommendation: recommendation, priority, difficulty, status: difficulty === "developer" ? "needs_developer" : "needs_approval", can_auto_fix: false, requires_approval: difficulty !== "developer", requires_developer: difficulty === "developer", what_to_do: steps, what_to_do_steps: steps, fix_steps: steps, who_can_do_this: owner, estimated_time: difficulty === "developer" ? "about 1–2 hours" : "about 10–30 minutes", time_estimate: difficulty === "developer" ? "about 1–2 hours" : "about 10–30 minutes", confidence_score: source === "url_evidence_layer" ? 55 : 88, source };
}

function calculateHealthScore({ pages, findings }) {
  let score = 92;
  const verifiedFailedPages = pages.filter((page) => isVerifiedFailedPage(page)).length;
  const suspiciousFailedPages = pages.filter((page) => page.status_code >= 400 || page.fetch_error).length - verifiedFailedPages;
...
function buildScanSummary({ pages, findings, healthScore, crawlResult }) {
  ...
  const routeBoundaryCandidates = pages.filter((page) => page.route_boundary_candidate).length;
  ...
  return { score: healthScore, status_label: healthScore >= 90 ? "Excellent" : healthScore >= 75 ? "Good" : healthScore >= 55 ? "Fair" : "Needs work", plain_english_summary: `FixList crawled ${pages.length} pages and found ${findings.length} grouped finding${findings.length === 1 ? "" : "s"}. ${verifiedFailedPages} verified page${verifiedFailedPages === 1 ? "" : "s"} failed or were blocked. ${suspiciousArtifacts} suspicious URL artifact${suspiciousArtifacts === 1 ? "" : "s"} need proof before being treated as broken links. ${routeBoundaryCandidates} route-boundary candidate${routeBoundaryCandidates === 1 ? "" : "s"} were sampled. Route-boundary risk is ${routeRisk}.${policyText}${accessWarning}`, pages_scanned: pages.length, pages_failed: failedPages, verified_failed_pages: verifiedFailedPages, suspicious_url_artifacts: suspiciousArtifacts, route_boundary_candidates_crawled: routeBoundaryCandidates, high_priority_count: highPriority, technical_issue_count: findings.length, queued_remaining: crawlResult.queued_remaining, crawl_warnings: crawlResult.warnings };
}
...
function buildUrlEvidenceSummary(pages) {
  ...
  const bySource = {};
  const byTemplate = {};
  const routeBoundary = { crawled: 0, indexable: 0, examples: [] };
  for (const page of pages || []) {
    ...
    byTemplate[key] = byTemplate[key] || { pages: 0, failed: 0, suspicious: 0, route_boundary_candidates: 0, examples: [] };
    ...
    if (page.route_boundary_candidate) { byTemplate[key].route_boundary_candidates += 1; routeBoundary.crawled += 1; if (page.indexable) routeBoundary.indexable += 1; if (routeBoundary.examples.length < 8) routeBoundary.examples.push(cleanPath(page.final_url || page.url || "")); }
    if (byTemplate[key].examples.length < 5) byTemplate[key].examples.push(cleanPath(page.final_url || page.url || ""));
  }
  return { by_confidence: byConfidence, by_source: bySource, by_template: byTemplate, route_boundary_candidates: routeBoundary };
}

function isVerifiedFailedPage(page) { return Boolean((page.status_code >= 400 || page.fetch_error) && page.url_confidence !== "crawler_artifact"); }
...
async function validatePublicHttpUrl(value) {
  ...
  if (typeof Deno.resolveDns === "function" && !isIpAddress(hostname)) {
    try { const records = await Deno.resolveDns(hostname, "A"); if (records.some((ip) => isPrivateIp(ip))) return { ok: false, reason: "DNS resolves to a private/internal IP." }; } catch {}
  }
  return { ok: true };
}
...
function matchesAnyPattern(path, patterns) { if (!Array.isArray(patterns) || patterns.length === 0) return false; const target = cleanPath(path).toLowerCase(); return patterns.some((pattern) => { const raw = String(pattern || "").trim().toLowerCase(); if (!raw) return false; if (!raw.includes("*")) return target.includes(raw); const regex = new RegExp(`^${raw.split("*").map(escapeRegExp).join(".*")}$`); return regex.test(target); }); }
function dequeueByPriority(queue) { if (queue.length === 0) return undefined; let bestIndex = 0; for (let i = 1; i < queue.length; i += 1) if ((queue[i].priority || 0) > (queue[bestIndex].priority || 0)) bestIndex = i; return queue.splice(bestIndex, 1)[0]; }
function withTimeoutPromise(promise, ms) { return Promise.race([Promise.resolve(promise), new Promise((_, reject) => setTimeout(() => reject(new Error("policy model timeout")), ms))]); }
