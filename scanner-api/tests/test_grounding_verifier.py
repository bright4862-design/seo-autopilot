...
def ann(annotation_id="a1", **overrides):
    payload = {
        ...
        "numeric_claims": [{"value": 88, "source_ref": "$.health_score"}],
        "fix_refs": ["fix-1"],
        "root_cause_refs": ["root-1"],
        "state_claims": [{"value": "complete", "source_ref": "$.scan_status"}],
    }
...
def answer(*annotations):
    return {"schema_version": "chat_answer_v2", "answer_id": "answer-1", "annotations": list(annotations)}
...
def test_fabricated_count_is_rejected_by_numeric_provenance():
    bad = ann(numeric_claims=[{"value": 89, "source_ref": "$.health_score"}])
    result = verify_grounded_payload(bad, sealed_l2=sealed_l2())
    assert result.status == "rejected"
...
def test_missing_numeric_source_is_rejected():
    bad = ann(numeric_claims=[{"value": 1, "source_ref": "$.invented.count"}])
    result = verify_grounded_payload(bad, sealed_l2=sealed_l2())
    assert result.status == "rejected"
...
def test_authority_or_state_claim_must_match_exact_source():
    bad = ann(state_claims=[{"value": "partial", "source_ref": "$.scan_status"}])
    result = verify_grounded_payload(bad, sealed_l2=sealed_l2())
    assert result.status == "rejected"
...
def test_deterministic_conflict_rejects_entire_answer():
    a1 = ann("a1")
    a2 = ann("a2", numeric_claims=[{"value": 87, "source_ref": "$.health_score"}])
