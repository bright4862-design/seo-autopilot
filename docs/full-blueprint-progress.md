...
- `main` was reverified on 2026-09-20 at `2ad64dc55ccc49d8fba4259f3b17ad6cdea643ad`, containing V7 runtime/public-build changes and durable ownership-before-admission fix #308.
...
## Stage 2 — B06–B18 source integration at whole-stage review gate
...
- **B13 — local entity completeness/status:** integrated as evidence-only. Accepted usable HTML produces bounded JSON-LD local-entity observations. Explicit structured status fields or conservative accepted title/H1 phrases can establish open/closed/Coming Soon context; arbitrary body wording cannot. Missing regular hours are contextual for verified Coming Soon/closed states, required for verified open, and unknown when applicability is unknown.
...
- **B15 — contextual freshness:** integrated as evidence-only. A fail requires current-content intent plus contradictory current-scoped temporal evidence. Old articles/years/dated paths alone do not fail; historical/archive wording remains historical; year-only dates use 31 December conservatively.
...
- labelled Stage-1 corpus passed and remains synthetic; `full_30_site_gate=not_assessed` remains unchanged;
...
The current documentation commits are above that executable checkpoint. Do not describe a docs-only head as code certification unless its own CI is separately green.
...
Stage 2 is **not yet recorded complete**. Source integration for B06–B18 has reached the whole-stage review gate, but the following remain mandatory before shared Stage-3 integration:

1. obtain a fresh independent review of the complete combined B06–B18 source shape, not merely historical/lane reviews;
2. reproduce and fix every material review finding with behavioral regressions rather than weakening tests;
3. run fresh exact-head FixList CI after any review correction;
4. keep any internal evidence-only B09/B10–B18 field internal unless a real producer → Review → signed authority → persisted rows → customer/card/handoff/export chain is intentionally added;
5. preserve all Standard-150/security/privacy/historical-reader invariants during that review hardening.
...
Do not integrate B19–B24 into shared ranking/authority/customer interfaces until Stage 2 is independently reviewed and exact-head green. Python Review remains canonical ranking authority.
...
Remain on Stage 2 and run the fresh independent whole-stage B06–B18 review. Correct any material issue with a reproducing regression and fresh exact-head CI. Only when that gate is genuinely green should the serialized owner begin integrating Stage 3 B19–B24 into the shared ranking/authority/persistence/customer path.
