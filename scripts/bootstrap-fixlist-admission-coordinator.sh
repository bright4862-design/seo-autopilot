...
OPERATOR_SA="${GCP_OPERATOR_SERVICE_ACCOUNT:-fixlist-github-operator@${PROJECT}.iam.gserviceaccount.com}"
...
if ! gcloud firestore databases describe --database="$DATABASE" --project="$PROJECT" >/dev/null 2>&1; then
  ...
  DB_JSON="$(mktemp)"; trap 'rm -f "$DB_JSON"' EXIT
...
WORKER_JSON="$(mktemp)"; trap 'rm -f "${DB_JSON:-}" "$WORKER_JSON"' EXIT
...
for ROLE in roles/run.sourceDeveloper roles/serviceusage.serviceUsageConsumer; do
  gcloud projects add-iam-policy-binding "$PROJECT" --member="serviceAccount:${OPERATOR_SA}" \
...
for TARGET_SA in "$COORDINATOR_SA" "$BUILD_SA"; do
  gcloud iam service-accounts add-iam-policy-binding "$TARGET_SA" --project="$PROJECT" \
    --member="serviceAccount:${OPERATOR_SA}" --role="roles/iam.serviceAccountUser" --condition=None --quiet >/dev/null
done
...
gcloud run services add-iam-policy-binding "$COORDINATOR" --project="$PROJECT" --region="$REGION" \
  --member="serviceAccount:${OPERATOR_SA}" --role="roles/run.developer" --condition=None --quiet >/dev/null
