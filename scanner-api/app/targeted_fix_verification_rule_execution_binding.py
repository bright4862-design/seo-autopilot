...
TARGETED_FIX_VERIFICATION_RULE_EXECUTION_BINDING_VERSION = (
    "targeted_fix_verification_rule_execution_binding_v2_fresh_execution_population"
)
TARGETED_FIX_VERIFICATION_EXECUTION_WINDOW_VERSION = "targeted_fix_verification_execution_window_v1_server_owned"
...
def _exact_timestamp(value: Any) -> int | None:
    if isinstance(value, bool) or not isinstance(value, int) or not 0 < value <= 2**53 - 1:
        return None
    return value


def _execution_window_error(
    window: Any,
    *,
    expected_execution_id: Any,
    evaluation_time_ms: Any,
    plan: dict[str, Any],
    scheduler_summary: Any,
    outcomes: Any,
) -> str:
    """Validate trusted producer time bounds; never establish caller authority.

    The protected service owns both the expected execution ID and the clock.
    A request body must never be allowed to supply either trusted argument.
    """
    fields = {"version", "execution_id", "plan_fingerprint", "started_at_ms", "expires_at_ms"}
    if not isinstance(window, dict) or set(window) != fields:
        return "execution_window_missing_or_invalid"
    if window.get("version") != TARGETED_FIX_VERIFICATION_EXECUTION_WINDOW_VERSION:
        return "execution_window_version_mismatch"
    execution_id = _exact_nonempty_string(expected_execution_id)
    if not execution_id or len(execution_id) > 128 or window.get("execution_id") != execution_id:
        return "execution_window_execution_id_mismatch"
    if window.get("plan_fingerprint") != plan.get("plan_fingerprint"):
        return "execution_window_plan_mismatch"
    started = _exact_timestamp(window.get("started_at_ms"))
    expires = _exact_timestamp(window.get("expires_at_ms"))
    now = _exact_timestamp(evaluation_time_ms)
    if started is None or expires is None or now is None:
        return "execution_window_timestamp_invalid"
    if expires <= started:
        return "execution_window_interval_invalid"
    if now < started:
        return "execution_window_not_started"
    if now >= expires:
        return "execution_window_expired"
    budget = scheduler_summary.get("request_budget") if isinstance(scheduler_summary, dict) else None
    seconds = budget.get("time_budget_seconds") if isinstance(budget, dict) else None
    if isinstance(seconds, bool) or not isinstance(seconds, (int, float)):
        return "execution_window_scheduler_time_budget_invalid"
    try:
        valid_seconds = math.isfinite(seconds) and seconds > 0
    except OverflowError:
        valid_seconds = False
    if not valid_seconds:
        return "execution_window_scheduler_time_budget_invalid"
    if (expires - started) / 1000 > seconds:
        return "execution_window_exceeds_scheduler_time_budget"
    if not isinstance(outcomes, list):
        return "recheck_outcomes_invalid"
    for outcome in outcomes:
        if not isinstance(outcome, dict):
            return "recheck_outcome_transport_invalid"
        if outcome.get("execution_id") != execution_id:
            return "recheck_outcome_execution_id_mismatch"
        observed = _exact_timestamp(outcome.get("observed_at_ms"))
        if observed is None or not started <= observed <= now:
            return "recheck_outcome_outside_execution_window"
    return ""
...
def build_rule_execution_bound_rule_evaluation_receipt(
    ...
    *,
    ...
    window_error = _execution_window_error(
        execution_window,
        expected_execution_id=expected_execution_id,
        evaluation_time_ms=evaluation_time_ms,
        plan=plan,
        scheduler_summary=preflight_scheduler_summary,
        outcomes=recheck_outcomes,
    )
    if window_error:
        return _failure(window_error)
    ...
    for result in rule_execution_results:
        ...
        finding_state = _exact_nonempty_string(result.get("finding_state"))
    ...
    material = {
        ...
        "execution_window": dict(execution_window),
        "model_calls_performed": False,
    }
    if any(
    ...
    return {
        **result,
        ...
        "rule_execution_binding": {
            ...
            "execution_window": expected["execution_window"],
        },
        "model_calls_performed": False,
