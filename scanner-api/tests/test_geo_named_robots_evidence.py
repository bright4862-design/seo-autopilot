...
@pytest.mark.parametrize('status,code', [('available', 403), ('available', 0), ('missing', 200)])
def test_http_status_cannot_contradict_observed_policy(status, code):
    with pytest.raises(ValueError, match='contradicts'):
        extract_named_robots_evidence(page(robots_txt_status=status, robots_txt_status_code=code))


def test_known_missing_file_cannot_contain_an_explicit_disallow():
    with pytest.raises(ValueError, match='contradicts'):
        extract_named_robots_evidence(page(robots_txt_status='missing', robots_txt_status_code=404,
                                          robots_txt_gptbot_allowed=False))
