...
@pytest.mark.parametrize('url', ['http://127.0.0.1/llms.txt', 'http://localhost/llms.txt', 'http://[::1]/llms.txt'])
def test_private_literal_identity_does_not_become_public_file_evidence(url):
    with pytest.raises(ValueError):
        extract_llms_txt_evidence(obs(url=url))


def test_html_response_is_not_misidentified_as_a_valid_markdown_file():
    result = extract_llms_txt_evidence(obs(content_type='text/html', body='# Search result\n'))
    assert result['presence'] == result['format_status'] == 'not_verified'


def test_invalid_protocol_relative_credentials_are_not_counted_as_resource_links():
    result = extract_llms_txt_evidence(obs(body='# Docs\n- [Bad](//user:pass@example.com/a)\n- [Good](/b)\n'))
    assert result['linked_resource_count'] == 1
