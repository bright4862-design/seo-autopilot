...
- `main` was refreshed during the B24 slice and remains `2ad64dc55ccc49d8fba4259f3b17ad6cdea643ad`, containing V7 runtime/public-build changes and durable ownership-before-admission fix #308.
...
### B22 — source helper only

Strict authenticated private-preview selection/whitelist helper exists in the reviewed lane. The actual V7 entitlement/customer preview seam and no-leak proof remain unwired while Stage-1 release is frozen. B22 is partial.
...
This run integrated the reviewed B24 helper into the real canonical Review -> completion-HMAC source path without creating a new customer route or touching V7 persistence.

**RED:** `37b73a98e6b54db9c86f0de9eb921ff178a0201a`, FixList CI `35515644839`. The new B24 signed-source regression failed in the Python scanner-api job while lint/typecheck/contracts/frontend stayed green because `stage3_handoff_v2_source` did not exist.

**Implementation:** `5eb4b6751019e98f87afc3b329880fbd33e0fdd6`. The shared integration now:

- requires exact trusted producer `scan_id == scan_run_id` before building the source;
- accepts root-cause identity only from B20 `grouping_state=verified` groups bound to that exact producer scan;
- carries family IDs from verified B20 partitions, with fallback only to already-stamped canonical family evidence;
- transports B19 priority factors plus B21 observation/population counts and existing canonical indexable count evidence;
- transports evidence references, verification steps, dependency/vendor owner and the real scanner user agent through the existing `build_handoff_v2` serializer;
- sets `operator_authorized=False`, so operator/debug `suppressed_findings` cannot enter this customer-safe source; internal B20 `suppressed_members` are not serialized by the allowlisted handoff contract;
- fails the source closed rather than guessing if one legacy canonical action maps to multiple different verified root-cause IDs;
- attaches the source before completion signing, so the existing completion HMAC authenticates it;
- does not create/copy/mutate the frozen V7 persistence or customer route.

CI `35516002420` remained red because the first fixture expected an indexable count to be derived without supplying canonical `indexable_affected`. That expectation was corrected rather than weakening unknown semantics: missing count evidence remains unknown instead of becoming zero or being inferred from page shape.

**GREEN executable checkpoint:** `790ebdc7ddec86a5a4bbfe4dfa0cb3ee23e0a353`, FixList CI `35516240329` **passed**. Root scanner regressions, the full Python scanner-api suite including both B24 signed-source tests, labelled Stage-1 synthetic corpus, frozen beta revision, production scanner image build, lint, typecheck, generated release contracts, frontend contracts and production frontend build all passed.

Detailed checkpoint: `docs/superpowers/plans/2026-09-20-stage3-b24-signed-handoff-source.md`.

B24 remains **partial**, not complete overall. Fresh independent review of this shared source seam is still required. After Stage-1 accepted-main reconciliation, the authenticated source must be persisted/read through the real V7 path, preserve historical-v1 reader compatibility, feed the authenticated customer/operator handoff surface, and prove no suppressed/operator/private leakage through real customer/card/export output.
...
- **B22 partial:** private-preview helper exists; real entitlement/customer seam and no-leak proof open.
...
- **B24 partial:** signed handoff-v2 source now GREEN; independent review plus real V7 persistence/read/customer/operator/export proof open.

Stage 3 is **in progress, not complete**.

CI has requested Node 20 in prior runs but hosted setup has resolved newer Node 20 patch versions; none of these runs is represented as exact Node 20.19.5 evidence unless that exact runtime is independently proven.
